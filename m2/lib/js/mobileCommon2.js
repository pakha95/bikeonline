	...
	if(review_data.length > 0) {
		...
		for(var i=0; i<review_data.length; i++) {
			...
			if(review_data[i].authdelete === 'Y') {
				add_html+= '	<a href="javascript:;" onclick="delete_qnaReview( \'del_review\', \''+review_data[i].m_no+'\', '+review_data[i].sno+');"><div class="del-btn">삭 제</div></a>';
			}
			...
			for(var j=0; j<review_data[i].reply.length; j++) {
				...
				if(review_data[i].reply[j].authdelete === 'Y') {
					add_html+= '	<a href="javascript:;" onclick="delete_qnaReview( \'del_review\', \''+review_data[i].reply[j].m_no+'\', '+review_data[i].reply[j].sno+');"><div class="del-btn">삭 제</div></a>';
				}
...
function makeGoodsReviewList(review_data) {
	...
	if(review_data.length > 0) {
		...
		for(var i=0; i<review_data.length; i++) {
			add_html+= '<tr class="title" onclick="view_content(this, '+review_data[i].sno+');">';
			add_html+= '	<td class="first img non-notice" data-goodsno="' + review_data[i].goodsno + '">'+review_data[i].img_html+'</td>';
...
function getQnaM2Data(isAll) {
	var data_param = "mode=get_qna";
	var item_cnt = ($("#qna-table .title .non-notice").length) ? $("#qna-table .title .non-notice").length : $("#qna-table .title").length;
...
function makeQnaM2List(qna_data, goodsno) {
	if(qna_data.length > 0) {
		...
		for(var i=0; i<qna_data.length; i++) {
			...
			add_html+= '	<td class="first img non-notice" data-goodsno='+qna_data[i].goodsno+'" >'+qna_data[i].img_html+'</td>';
			add_html+= '	<td class="left last" >';
			add_html+= '		<div class="answer-yn">';
			...
			if(qna_data[i].accessable) {
				...
				if(qna_data[i].authdelete === 'Y') {
					add_html+= '	<a href="javascript:;" onclick="delete_qnaReview( \'del_qna\', \''+qna_data[i].m_no+'\', '+qna_data[i].sno+' );"><div class="del-btn">삭 제</div></a>';
				}
				add_html+= '	</div>';
				...
				for(var j=0; j<qna_data[i].reply.length; j++) {
					...
					add_html+= '	<a href="javascript:;" onclick="delete_qnaReview( \'del_qna\', \''+qna_data[i].reply[j].m_no+'\', '+qna_data[i].reply[j].sno+' );"><div class="del-btn">삭 제</div></a>';
					}
					add_html+= '			</div>';
				}
			}
				...
				if(qna_data[i].m_no > 0) {
					...
					add_html+= '		비밀번호 : <input type="password" id="goods-qna-password-'+qna_data[i].sno+'" name="password" required="required"/><button type="button" data-sno="'+qna_data[i].sno+'"  class="goods-qna-certification">확인</button>';
				}
				add_html+= '</div>';
