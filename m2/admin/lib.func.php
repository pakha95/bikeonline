...
function chkAdmin(){
	...
	if($data[level] != 100 && count($arr)>=1){
		...
		foreach ($t_arr as $dirnm){
			if (in_array($dirnm, $keys)) $arr = array_merge($arr, $combine_menu[$dirnm]);
		}
...
function naver_goods_diff($goodsno,$ar_update,$class="U") {
	...
	$ar_result = array();
	$ar_goods_field=array('goodsnm','img_l','brandno','origin','maker','delivery_type','goods_delivery','use_emoney','usestock','runout','open');
