...
if($cfgCoupon['coupon'] && $_POST['couponVersion']) {
...
$coupon_item = $coupon_price->get_goods_coupon_mobile('order');
if($_POST['couponVersion']) $coupon_price->discountGoodsPrice($cart->item);
...
if($coupon_price->arCoupon && $sess['m_no']){
		...
		if($coupon_price->arCoupon)foreach($coupon_price->arCoupon as $arCoupon){
			if(!is_array($_POST['apply_coupon']) || !in_array($arCoupon[sno],$_POST['apply_coupon'])) continue;
			unset($couponDc, $couponEmoney, $realcouponDc);
			...
			$add = "";
			if($_POST['couponVersion']) {
				$realcouponDc = array_sum($coupon_price->goodsDc[$arCoupon['couponcd']]);
				$add = "realdc		= '$realcouponDc',";
			}
...
insert into ".GD_ORDER." set $qrTmp
	...
	referer			= '$referer',
	memo			= '$_POST[memo]',
	memoModel			= '$_POST[memoModel]',
	memoModelYear			= '$_POST[memoModelYear]',
	memoJacket			= '$_POST[memoJacket]',
	memoPants			= '$_POST[memoPants]',
...
foreach ($cart->item as $k=>$item){
	...
	$all_order_cnt += $item[ea];
}

### 모바일 통계 데이터 쌓기 ###
$ins_analysis = array();
$ins_analysis['order_cnt'] = $all_order_cnt;
$ins_analysis['order_price'] = $settleprice;
$ins_analysis['reg_date'] = date('Y-m-d H:i:s');
$analysis_query = $db->_query_print('INSERT INTO '.GD_MOBILE_ANALYSIS.' SET [cv]', $ins_analysis);
$db->query($analysis_query);
