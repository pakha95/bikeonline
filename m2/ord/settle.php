...
//한글깨짐 수정 추가
$_POST['nameOrder'] = iconv('utf-8','euc-kr',$_POST['nameOrder']);
$_POST['nameReceiver'] = iconv('utf-8','euc-kr',$_POST['nameReceiver']);
$_POST['address'] = iconv('utf-8','euc-kr',$_POST['address']);
$_POST['address_sub'] = iconv('utf-8','euc-kr',$_POST['address_sub']);
...
if($_POST['updateMemberInfo']=='y' && $sess[m_no]){
	...
	$db->query($query);
} else if($_POST['delivery_default'] == 'y' && $sess['m_no']) {
	$cnt_query = "SELECT COUNT(*) FROM ".GD_MEMBER_DELIVERY." dm WHERE dm.gmd_m_no = ".$sess['m_no'];
	...
	if ($cnt < 10) {
		...
		INSERT INTO ".GD_MEMBER_DELIVERY." SET
			gmd_m_no = '".$sess['m_no']."',
			gmd_default = 'y',
			gmd_title = '".$_POST['nameReceiver']."',
			gmd_name = '".$_POST['nameReceiver']."',
...
} else if($_POST['delivery_add'] == 'y' && $sess['m_no']) {
	$cnt_query = "SELECT COUNT(*) FROM ".GD_MEMBER_DELIVERY." dm WHERE dm.gmd_m_no = ".$sess['m_no'];
	list ($cnt) = $db->fetch($cnt_query);
	if($cnt < 10) {
		if($cnt == 0) {
			...
			INSERT INTO ".GD_MEMBER_DELIVERY." SET
				...
				gmd_zipcode = '".(str_replace('-', '', $member['zipcode']) ? $member['zipcode'] : $_POST['zipcode'][0]."-".$_POST['zipcode'][1])."',
				gmd_address = '".($member['address'] ? $member['address'] : $_POST['address'])."',
				gmd_road_address = '".$_POST['road_address']."',
				gmd_address_sub = '".$_POST['address_sub']."',
				gmd_phone = '".(str_replace('-', '', $member['phone']) ? $member['phone'] : $_POST['phoneReceiver'][0]."-".$_POST['phoneReceiver'][1]."-".$_POST['phoneReceiver'][2])."',
				gmd_mobile = '".(str_replace('-', '', $member['mobile']) ? $member['mobile'] : $_POST['mobileReceiver'][0]."-".$_POST['mobileReceiver'][1]."-".$_POST['mobileReceiver'][2])."',
				gmd_zonecode = '".$_POST['zonecode']."',
				gmd_regdate = NOW()
			";
		} else {
