...
if ($sess){
	...
	// 최근 주소지
	$lastDelivery = "
		SELECT nameReceiver AS last_name, road_address AS last_road_address, zipcode AS last_zipcode, address AS last_address, phoneReceiver AS last_phone, mobileReceiver AS last_mobile, zonecode AS last_zonecode
		FROM ".GD_ORDER."
		WHERE m_no='$sess[m_no]' AND step >= 1 AND step2 >= 0 
		ORDER BY orddt DESC
		LIMIT 1
	";
	$last = $db->fetch($lastDelivery,1);
	...
	if($cnt == 0) {
		if(str_replace('-', '', $member['zipcode']) || str_replace('-', '', $member['phone']) || str_replace('-', '', $member['mobile'])) {
			...
			INSERT INTO ".GD_MEMBER_DELIVERY." SET
				...
				gmd_zonecode = '".$member['zonecode']."',
				gmd_regdate = NOW()
			";
			$db->query($q_insert);
		}
	}
	$q_select = "SELECT * FROM ".GD_MEMBER_DELIVERY." dm WHERE dm.gmd_m_no = ".$sess['m_no']." ORDER BY dm.gmd_default DESC, dm.gmd_sno DESC";
...
// 최근 주소지
if ($last){
	$last['last_zipcode'] = explode("-",$last['last_zipcode']);
	$last['last_phone'] = explode("-",$last['last_phone']);
	$last['last_mobile'] = explode("-",$last['last_mobile']);
	$tpl->assign($last);
}
