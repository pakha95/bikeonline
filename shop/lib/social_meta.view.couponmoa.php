<?
$_meta_site_info['couponmoa'] = array('name'=>'ÄíÆù¸ð¾Æ','url'=>'http://www.couponmoa.com/');

// ÄíÆù¸ð¾Æ ºä
class social_meta_view_couponmoa extends social_meta_view {

	var $data = array();

	function social_meta_view_couponmoa() {
		$this->goods_only = 'A';
		$this->encode = 'UTF-8';
	}
	...
	function make( $data = array() ) {
		...
		for($i=0,$max=sizeof($keys);$i<$max;$i++) {
			...
			$row['usestartdt'] = date('Y-m-d', strtotime($row['usestartdt']));
			$row['useenddt'] = date('Y-m-d', strtotime($row['useenddt']));
