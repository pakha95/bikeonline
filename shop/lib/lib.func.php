...
function ctlStep($ordno,$step,$stock='')
{
	...
	if ($pre[step]!=$step){
		...
		switch ($step){
			...
			case "1": case "2":		// 입금확인, 배송준비중 = > 입금확인, 해외주문/배송준비중
		...
		if ($exec_dyn_y){
			...
			if ($pre[inflow] == 'interpark');
				...
				if ($todayshop_noti !== false) {
					...
					### 발송확인SMS ### 수정 by jung
					$GLOBALS[dataSms] = $pre;
					if($pre[deliverycomp]  == "미국->한국 운송중"){
						sendSmsCase2('delivery',$pre[mobileOrder]);
					}else if($pre[deliverycomp]  == "기타(방문수령등)"){
					}else{
...
function setStock($ordno)
{
	...
	$db->query("LOCK TABLES
				...
				".GD_MEMBER." write,
				".GD_SMS_LOG." write,
				".GD_SMS_SENDLIST." write,
				".GD_KAKAOALIMTALK_TEMPLATE." write,
				".GD_KAKAOALIMTALK_SENDLIST." write,
				".GD_KAKAOALIMTALK_SENDLOG." write
				");
...
{
	global $r_sendDateCode, $r_sendDateDefault, $kakaoAlimtalk;

if (!$kakaoAlimtalk) {
	@include_once dirname(__FILE__)."/../lib/kakaoAlimtalk.class.php";
	$kakaoAlimtalk = new kakaoAlimtalk();
}

$alimtalkCheck = $kakaoAlimtalk->alimtalkConfigCheck($case);
if ($alimtalkCheck === true) {
	extract($kakaoAlimtalk->config[$case]);
}
else {
	$maxLen = 90;
	...
	if ($alimtalkCheck === true) {
		// 사용자에게 전송
		if ($send_c && $mobile && $memTemplate) {
			$kakaoAlimtalk->sendAlimtalk($case, $memTemplate, $mobile);
		}

		// 관리자에게 전송
		if ($send_a && $GLOBALS['cfg']['smsAdmin'] && $adminTemplate) {
			$kakaoAlimtalk->sendAlimtalk($case, $adminTemplate, $GLOBALS['cfg']['smsAdmin']);
		}

		// 추가관리자에게 전송
		if ($send_m && $GLOBALS['cfg']['smsAddAdmin'] && $adminTemplate) {
			$smsAddAdmin = explode("|",$GLOBALS['cfg']['smsAddAdmin']);
			foreach ($smsAddAdmin as $key=>$val){
				$kakaoAlimtalk->sendAlimtalk($case, $adminTemplate, $val);
			}
		}
	}
	else {
...
### 상황별 SMS 발송 추가 by jung
function sendSmsCase2($case,$mobile)
{
	global $r_sendDateCode, $r_sendDateDefault,$kakaoAlimtalk;
	...
	if ($alimtalkCheck === true) {
		...
		$maxLen = 90;
		@include dirname(__FILE__)."/../conf/sms/dcode.php";
		$send_c = "on";
		if (is_array($sms_auto)) extract($sms_auto);
	}
...
### EUC-KR 필터링
function remainOnlyEucKr($str)
{
$regEn = '/^[\x00-\x7F]/';	// ASCII 코드
$regSp = '/^[\xA1-\xAC][\xA1-\xFE]/';	// 특수문자, 자음, 모음, 로마자, 히라가나, 가타카나, 러시아어
$regKr = '/^[\xB0-\xC8][\xA1-\xFE]/';	// 한글 2350자
$regHan = '/^[\xCA-\xFD][\xA1-\xFE]/';	// 한문
$rts = '';
...
elseif(preg_match($regHan, $str) === 1)
{
	$rts .= substr($str, 0, 2);
	$str  = substr($str, 2);
}
elseif(preg_match($regSp, $str) === 1)
{
	$rts .= substr($str, 0, 2);
	$str  = substr($str, 2);
}
...
function displayDelivery($deliveryno,$deliverycode){
	...
	if($deliverycomp == "DHL"){
		echo "<iframe src=\"".$deliveryurl.$deliverycode."\" frameborder=\"0\" width=\"600\" height=\"300\" marginwidth=\"0\" marginheight=\"0\" scrolling=\"yes\">";
	}else{
	echo "<div class=\"title title_top\">배송추적</div>
	<b style='font-size:1.5em'>$deliverycomp : $deliverycode</b><br>";
	if($deliverycomp == "미국->한국 운송중"){
		echo "<br><div style='font-size:15px'>미국에서 한국으로 오는 운송은 국제무역운송인 관계로 배송 추적이 불가능합니다.</div>";
	}
...
### 쿠폰정보
function getCouponInfo($goodsno,$goodsprice='',$mode='',$daumCpc='', $except=''){
	global $tpl;
	...
	if($cfgCoupon['coupon'] && is_file($tpl->template_dir.'/proc/popup_coupon_division.htm')) {
	...
	if ($couponVersion === true) {
		...
		$goodsModel    = $goodsHelper->getGoodsModel($goodsno);
		$discountModel = Clib_Application::getModelClass('Goods_Discount');
		$param = array(
			'goodsno' => $goodsno,
			'price' => $goodsprice,
		);
		$goodsDiscount = $discountModel->getDiscountAmountSearch($param);

		if (in_array($cfgCoupon['range'],array('0','1'))) {
			...
			if ( ! $goodsModel['exclude_member_discount'] && $group_profit->dc_type == 'goods') {
				if ($goodsprice >= $group_profit->dc_std_amt) {
					if ( ! $mdc_exc)
						$memberdc = getDcprice($goodsprice, $member['dc'] . "%");
				}
			}
		}
		switch($except) {
		...
		if ($goodsprice - $memberdc - $goodsDiscount < $result[0]) $result[0] = $result[10] = $goodsprice - $memberdc - $goodsDiscount;
	}
	return $result;
}
...
function setNotAvailbleGoodsName($goodsno, $msg = null, &$goodsnames, $isMobile = '') {
...
// 쿠폰정보 모바일샵
function getCouponInfoMobile($goodsno,$goodsprice='',$mode='',$daumCpc='', $except=''){
	global $set, $tpl;
	...
	$cfgCoupon = & $GLOBALS['cfgCoupon'];
	if(!$cfgCoupon) @include SHOPROOT."/conf/coupon.php";
	if($cfgCoupon['coupon'] && is_file($tpl->template_dir.'/proc/coupon_list_division.htm')) {
		$coupon_price = Core::loader('coupon_price_division');
		$couponVersion = true;
	} else {
		$coupon_price = Core::loader('coupon_price');
...
/**
 * 지번, 도로명주소 비교 (시/도, 시/군/구, 도로명주소 비교)
 * @param string $jibunAddress 지번주소
 ...
 * false - 불일치
 */
function checkMissJuso($jibunAddress, $roadAddress)
{
	$res = true;

	$missJusoFile = dirname(__FILE__).'/MissJuso.class.php';
	if(is_file($missJusoFile)){
		include $missJusoFile;
		$missJuso = new MissJuso();
		$res = $missJuso->check($jibunAddress, $roadAddress);
	}
