 ...
 * workingby         2015.11.26        add auto sms.
 */
set_time_limit(0);
...
class dormant
{
	 ...
	 * @return boolean true - 휴면회원O 일반회원으로 정상복원되었을시, false - 휴면회원X , 정상복원 되지 않았을시
	 * @date 2015-10-29
	 */
	...
	public function checkAutoSmsExecuteAble()
	{
		...
		$alimtalkUse = $kakaoAlimtalk->alimtalkConfigCheck('dormant');
		if ($alimtalkUse === true) {
			$sms_auto = $kakaoAlimtalk->config['dormant'];
		}
		else {
	...
	public function executeDormantSms()
	{
		try {
			...
			$alimtalkUse = $kakaoAlimtalk->alimtalkConfigCheck('dormant');
			if ($alimtalkUse === true) {
				 $errorMessage = self::sendDormantKakaoAlimtalk();
				 if($errorMessage != ''){
					throw new Exception($errorMessage);
				}
				self::writeLog('3step', 'STEP1. KAKAOALIMTALK SEND OK!');
			}
			//SMS 발송
			else {
	 ...
	 * SMS 발송(예약발송)
	 * @author workingby <bumyul2000@godo.co.kr>
	 * @param array $member, array $sms_auto
	 * @return boolean
	 * @date 2016-03-31
	 */
	private function sendSmsReserve($member, $sms_auto)
	{
		global $sms, $sms_sendlist;

		$_msg = '';
		$GLOBALS['dataSms'] = $smsData = $msg = $sendListInsertArray = array();

		try {
			if(!is_object($sms)){
				include dirname(__FILE__).'/sms.class.php';
				$sms = new Sms();
			}
			if(is_object($sms) && !is_object($sms_sendlist)){
			...
			$sendListInsertArray[] = array(
				'sms_memNo' => $member['m_no'],
				'sms_name'	=> $member['name'],
				'phone'		=> $member['mobile'],
			);

			//메시지 파싱
			$GLOBALS['dataSms'] = $smsData;
			$_msg = parseCode($sms_auto['msg_c']);
			if(!trim($_msg)){
				throw new Exception();
			}

			//SMS 예약 시간
			$reserveTime = self::getReserveTime($sms_auto);

			if(function_exists("gd_str_split")){
				$msg = gd_str_split($_msg, 90);
			}
			else {
				$msg[0] = $_msg;
			}
			...
			for ($i=0; $i<$msg_size; $i++) {
				if($i!=0){
					if($GLOBALS['cfg']['smsAutoSendType'] != 'MULTI'){
						break;
					}
					$reserveTime = date("Y-m-d H:i:s", strtotime($reserveTime. " +1 seconds"));
				}

				$sms->log($msg[$i], $member['mobile'], 'dormant', 1, $reserveTime);
				$sms_sendlist->sms_mode		= $sms_sendlist->getSms_mode(1);
				$sms_sendlist->sms_logNo	= $sms->smsLogInsertId;
				$sms_sendlist->setListInsert($sendListInsertArray);
				$sms->send($msg[$i], $member['mobile'], $GLOBALS['cfg']['smsRecall'], $reserveTime, '', 'res_send');
				$sms->update_ok_eNamoo = true;
				$sms->update();
				unset($sms->update_ok_eNamoo);
			}

			unset($sms, $sms_sendlist);
	 ...
	 * SMS 예약발송 시간
	 * @author workingby <bumyul2000@godo.co.kr>
	 * @param array $sms_auto
	 * @return string
	 * @date 2016-03-31
	 */
	private function getReserveTime($sms_auto)
	{
		$reserveTime = $reserveTime_today_before = $reserveTime_tomorrow = '';
		if((int)$sms_auto['sendTime'] == 0){
			//디폴트값 9시
			$sms_auto['sendTime'] = 9;
		}
		if(strlen($sms_auto['sendTime']) == 1){
			$sms_auto['sendTime'] = str_pad($sms_auto['sendTime'], 2, '0', STR_PAD_LEFT);
		}

		$reserveTime = date("Y-m-d ".$sms_auto['sendTime'].":00:00");
		$reserveTime_today_before = strtotime($reserveTime . " -10 minute");
		$reserveTime_tomorrow = strtotime($reserveTime ." +1 day");

		if(strtotime("now") < $reserveTime_today_before){
			return $reserveTime;
		}
		else {
			return date("Y-m-d H:i:s", $reserveTime_tomorrow);
		}
	}

	/**
	 * 알림톡 발송
	 * @author workingby <dong7330@godo.co.kr>
	 * @param void
	 * @return string
	 * @date 2017-10-11
	 */
	private function sendDormantKakaoAlimtalk()
	{
		global $db;
		...
		try {
			...
			if($dormantSmsCount_30 > 0 && $sms_auto['sendBeforeDay_30'] == 'y' && $sms_auto['send_c'] == 'on'){
				...
				$res = $db->query($query);
				if(!$res){
					throw new Exception("Query Error - fail to load auto alimtalk 30 Query.");
				}

				$GLOBALS['dataSms'] = array();

				while($member = $db->fetch($res, 1)){
					$smsData = array(
						'name'=>$member['name'],
						'id' => $member['m_id'],
						'toBeDate' => date("Y-m-d", strtotime($member['last_login'] . " +1 year")),
					);

					$GLOBALS['dataSms'] = $smsData;

					$phoneNumber = str_replace("-", "", $member['mobile']);
					if ($phoneNumber) {
						$sendTime = self::getReserveTime($sms_auto);
						$sendResult = $kakaoAlimtalk->sendAlimtalk('dormant', $sms_auto['memTemplate'], $phoneNumber, $sendTime, $member['m_no']);
						if ($sendResult[$member['m_no']] == 'success') {
							$this->sendSms_successID_30[] = $member['m_id'];
						}
					}
				}
			...
			if($dormantSmsCount_7 > 0 && $sms_auto['sendBeforeDay_7'] == 'y' && $sms_auto['send_c'] == 'on'){
				...
				$res = $db->query($query);
				if(!$res){
					throw new Exception("Query Error - fail to load auto alimtalk 7 Query.");
				}
				while($member = $db->fetch($res, 1)){
					$smsData = array(
						'name'=>$member['name'],
