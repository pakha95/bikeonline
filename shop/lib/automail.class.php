...
class automail {
	...
	function _assign_tpl($ordno) {
		...
		$query = "
		SELECT a.sno,a.goodsno,a.goodsnm,a.opt1,a.opt2,a.addopt,a.reserve,a.price,a.ea,b.img_s
		...
		$row[deliveryno]=$data[deliveryno];	//택배사번호
		...
		if($row[deliveryno] != "105" && $row[deliveryno] != "106" ){
			$deliveryLink='<a href="'.$row[deliveryurl].$row[deliverycode].'" target=_blank>'.$row[deliverycode].'</A>';			
		}else{
			$deliveryLink='<a href="'.$row[deliveryurl].$sno[0].'" target=_blank>'.$row[deliverycode].'</A>';
		}
