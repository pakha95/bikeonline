...
class Mobile2GoodsDisplay
{
	...
	function initializeMainDisplay($onlyChkOn=false)
	{
		$where  = '';
		if ($onlyChkOn === true)  $where .= " and chk = 'on'";

		$designData = $this->_dbo->_select('SELECT * FROM gd_mobile_design WHERE page_type="main" ' . $where . ' ORDER BY mdesign_no ASC');
		list($cntData) = $this->_dbo->fetch('SELECT count(mdesign_no) as cnt FROM gd_mobile_design WHERE page_type="main"');
	...
	{
		global $cfgMobileShop,$cfg;
		$displaySequence = $this->getDisplaySequence($mdesignNo);
		$pcDesignData = $this->getPCDesignData($displaySequence-1);
		$more_exists = file_exists(dirname(__FILE__).'/../data/skin_mobileV2/'.$cfg['tplSkinMobile'].'/goods/list/tpl_08.htm');	// 스킨패치 여부 확인

		if ($design['chk'] == 'on') {
			if ($cfgMobileShop['vtype_main_pc_disp'] == 1 && $more_exists) $design['tpl'] = 'tpl_08';	// 상품 더보기형
			else $design['tpl'] = 'tpl_03';	// 상품 스크롤형
	...
	function getPCMainDisplayGoods($mdesignNo, $isAdmin = false)
	{
		...
		if (!$cfg_step_arr['sort_type'] || $cfg_step_arr['sort_type'] == 1) {
			$goodsDisplayQuery = $this->_dbo->_query_print("
				SELECT gd.goodsno, g.model_name, g.goodsnm, g.img_mobile, g.img_l, g.img_m, g.use_mobile_img, g.img_w, g.strprice, go.price, go.consumer, g.runout, g.usestock, g.totstock, g.use_only_adult, g.speach_description_useyn, g.speach_description, g.use_goods_discount $_add_field
		...
		if (!$displayGoods = $dbCache->getCache($goodsDisplayQuery)) {
			...
			while ($row = $this->_dbo->fetch($res, true)) {
				...
				// 소비자가
				if (strlen($row['strprice']) > 0) { // 가격대체문구 있는 경우 초기화
					$consumer = '';
				}
				else if ($row['consumer'] > 0) {
				...
				else if($row['use_goods_discount']){
				...
				$goodsDiscountPrice = '';
				if (strlen($row['strprice']) > 0) { // 가격대체문구 있는 경우 초기화
					$oriPrice = $goodsDiscountPrice = '';
				}
				else if ($displayCfg['displayType'] === 'discount') {
					$goodsDiscount = '';
	...
	function getMobileMainImg($cfg, $row, $is_main = true) {
		...
		} else if (preg_match("/^http/i", $mobile_pc_img)) {
			$goods_img = $mobile_pc_img;
		} else {
			if ($row['use_mobile_img'] === '1') {
	...
	function getMobileMainDisplayGoods($req_arr) {
		...
		//정렬
		$_add_table = $_add_field = $orderby2 = "";
		...
		// 제외시키지 않는 다면, 맨 뒤로 보낼지를 결정
			...
			if (!$req_arr['sort_type'] || $req_arr['sort_type'] == 1) {
				if($req_arr['display_type'] == '2') {
					$orderby2 = "order by `soldout` ASC, gd_mobile.md_sort, gd_mobile.dis_sort";
					$_add_field .= ",IF (g.runout = 1 , 1, IF (g.usestock = 'o' AND g.totstock = 0, 1, 0)) as `soldout`";
				} else {
		...
		if (!$req_arr['sort_type'] || $req_arr['sort_type'] == 1) {
			switch ($req_arr['display_type']) {
				case '1' :
					$tmp_query = "
						SELECT
							md.goodsno, g.model_name, g.goodsnm, g.img_i, g.img_s, g.img_m, g.img_l, g.use_mobile_img, g.img_w, g.img_pc_w, g.strprice, go.price, go.consumer, g.runout, g.usestock, g.totstock, g.use_only_adult, g.speach_description_useyn, g.speach_description, g.use_goods_discount $_add_field
						FROM
					...
					if (!$res_display = $dbCache->getCache($display_query)) {
						...
						if(is_array($tmp_display) && !empty($tmp_display)) {
							foreach($tmp_display as $row_display) {
								$tmp_arr = array();
								$tmp_arr['goods_prefix'] = $row_display['goods_prefix'];
								...
								if (strlen($row_display['strprice']) > 0) {
								...
								// 소비자가
								if (strlen($row_display['strprice']) > 0) { // 가격대체문구 있는 경우 초기화
									$tmp_arr['consumer'] = '';
								}
								else if ($row_display['consumer'] > 0) {
								...
								// 상품할인
								if (strlen($row_display['strprice']) > 0) { // 가격대체문구 있는 경우 초기화
									$tmp_arr['special_discount'] = '';
								}
								else if($row_display['use_goods_discount']){
				...
				case '2' :
					$tmp_query = "
						SELECT * FROM (
							SELECT
								md.category, g.goodsno, g.model_name, g.goodsnm, g.img_i, g.img_s, g.img_m, g.img_l, g.use_mobile_img, g.img_w, g.img_pc_w, g.strprice, go.price, go.consumer, g.runout, g.usestock, g.totstock, g.use_only_adult, g.speach_description_useyn, g.speach_description, g.use_goods_discount $_add_field
							FROM
								".GD_MOBILE_DISPLAY." md
				...
				case '5' :
					...
					if(is_array($tab_info['tab_name']) && !empty($tab_info['tab_name'])) {
						foreach ($tab_info['tab_name'] as $key => $val) {
							$tmp_query = "
								SELECT
									md.goodsno, g.model_name, g.goodsnm, g.img_i, g.img_s, g.img_m, g.img_l, g.use_mobile_img, g.img_w, g.img_pc_w, g.strprice, go.price, go.consumer, g.runout, g.usestock, g.totstock, g.speach_description_useyn, g.speach_description, g.use_goods_discount $_add_field
								FROM
									".GD_MOBILE_DISPLAY." md
									LEFT JOIN ".GD_GOODS." g ON md.goodsno = g.goodsno
									LEFT JOIN ".GD_GOODS_OPTION." go ON md.goodsno = go.goodsno AND link and go_is_deleted <> '1' and go_is_display = '1'
	...
	function getMobileEventDisplayGoods($mevent_no, $event_group_sno = '', $display = '') {
		@include dirname(__FILE__). "/../../shop/conf/config.soldout.php";
		include_once dirname(__FILE__). "/../../shop/lib/json.class.php";
		...
		if ($display == 'G') {
			//$design_query = $this->_dbo->_query_print('SELECT tpl FROM '.GD_EVENT_GROUP.' WHERE group_sno=[s] AND sno=[i]', $mevent_no, $event_group_sno);
			$design_query = $this->_dbo->_query_print('SELECT tpl FROM '.GD_EVENT_GROUP.' WHERE sno=[i]', $event_group_sno);
			$where = " AND mevent_group_no=[i]";
		} else {
		...
		switch ( $res_design[0]['tpl']) {
			case "tpl_05":
				...
				if(is_array($tab_info['tab_name']) && !empty($tab_info['tab_name'])) {
					foreach ($tab_info['tab_name'] as $key => $val) {
						$tmp_query = "
							SELECT
								md.goodsno, g.goodsnm, g.model_name, g.img_s, g.img_i, g.img_m, g.img_l, g.use_mobile_img, g.img_x, g.img_pc_x, g.strprice, go.price, go.consumer, g.use_goods_discount, g.speach_description_useyn, g.speach_description, g.runout, g.usestock, g.totstock $_add_field
							FROM
								".GD_MOBILE_DISPLAY." md
								LEFT JOIN ".GD_GOODS." g ON md.goodsno = g.goodsno
								LEFT JOIN ".GD_GOODS_OPTION." go ON md.goodsno = go.goodsno AND link and go_is_deleted <> '1' and go_is_display = '1'
							WHERE
								md.mevent_no=[s] and g.open_mobile AND md.tab_no=[s]
								$where
							$orderby
							";
			...
			default:
				$tmp_query = "
					SELECT
						md.goodsno, g.model_name, g.goodsnm, g.img_s, g.img_i, g.img_m, g.img_l, g.use_mobile_img, g.img_x, g.img_pc_x, g.strprice, go.price, go.consumer, g.use_goods_discount, g.speach_description_useyn, g.speach_description, g.runout, g.usestock, g.totstock $_add_field
					FROM
						".GD_MOBILE_DISPLAY." md
						LEFT JOIN ".GD_GOODS." g ON md.goodsno = g.goodsno
						LEFT JOIN ".GD_GOODS_OPTION." go ON md.goodsno = go.goodsno AND link and go_is_deleted <> '1' and go_is_display = '1'
					WHERE
						md.mevent_no=[s] and g.open_mobile
						$where
					$orderby
					";

				if ($display == 'G') {
					$display_query = $this->_dbo->_query_print($tmp_query, $mevent_no, $event_group_sno);
				} else {
