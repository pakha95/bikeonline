...
class e2i_order_api extends putLog
{
	...
	var $interfaceurl = 'https://godointerpark.godo.co.kr/e2i.orderApi.php?method=%s';	# 인터페이스 URL
	...
	function e2i_order_api($interfaceid, $data)
	{
		...
		switch ($this->interfaceid)
		{
			...
			case "cnclOutOfStockForComm": # 품절주문취소요청
				$this->cnclOutOfStockForComm($data);
				break;
			...
			case "clmReqAcceptForComm": # 반품요청승인
				$this->clmReqAcceptForComm($data);
				break;

			case "exchangeReqAcceptForComm": # 교환요청승인
				$this->exchangeReqAcceptForComm($data);
				break;

			case "clmReqRefuseForComm": # 반품요청거부
				$this->clmReqRefuseForComm($data);
				break;

			case "exchangeReqRefuseForComm": # 교환요청거부
				$this->exchangeReqRefuseForComm($data);
				break;

			case "clmStoreCompForComm": # 반품/교환입고확정
				$this->clmStoreCompForComm($data);
				break;

			case "exchangeCompForComm": # 교환확정
				$this->exchangeCompForComm($data);
				break;
		}
	...
	function delvCompForComm($data)
	{
		...
		$this->log("delvCompForComm START");
		...
		if ($gOrder['ordno'] == ''){
		...
		else if ($gOrder['inflow'] != 'interpark'){
			$this->log("Fail : Not order of Interpark");
		}
			...
			while ($gItem = $GLOBALS['db']->fetch($res))
			{
				...
				$url = sprintf("%s&orderclm_no=%s&ord_seq=%s&delv_dt=%s&delv_comp=%s&delv_no=%s", $this->interfaceurl, $gOrder['inpk_ordno'], $gItem['inpk_ordseq'], $gOrder['ddt'], $gItem['dvno'], $gItem['dvcode']);
	...
	### 품절주문취소요청
	function cnclOutOfStockForComm($data)
	{
		$this->callCnt++;
		$data['ordno'] = trim($data['ordno']);
		$this->log("cnclOutOfStockForComm START");
		...
		$this->log("cnclOutOfStockForComm END");
	...
	### 반품요청승인
	function clmReqAcceptForComm($data)
	{
		$this->callCnt++;
		$this->log("clmReqAcceptForComm START");
		...
		$this->log("clmReqAcceptForComm END");
	...
	### 교환요청승인
	function exchangeReqAcceptForComm($data)
	{
		$this->callCnt++;
		$this->log("exchangeReqAcceptForComm START");
		...
		$this->log("exchangeReqAcceptForComm END");
	...
	### 반품요청거부
	function clmReqRefuseForComm($data)
	{
		$this->callCnt++;
		$this->log("clmReqRefuseForComm START");
		...
		$this->log("clmReqRefuseForComm END");
	...
	### 교환요청거부
	function exchangeReqRefuseForComm($data)
	{
		$this->callCnt++;
		$this->log("exchangeReqRefuseForComm START");
		...
		$this->log("exchangeReqRefuseForComm END");
	...
	### 반품/교환입고확정
	function clmStoreCompForComm($data)
	{
		$this->callCnt++;
		$this->log("clmStoreCompForComm START");
		...
		if ($gClaim['clm_seq'] == ''){
			...
			if ($gClaim['clm_tpnm'] == '반품')
			{
				...
				$url .= sprintf("&supply_entr_no=%s&supply_ctrt_seq=%s", $this->inpkCfg['entrNo'], $this->inpkCfg['ctrtSeq']);
			}
			$this->log(sprintf("URL : %s", preg_replace("/^(http|https):\/\/[^\/]*\//", "/", $url)));
			...
			if (in_array($arrXml['ORDER_LIST']['RESULT']['CODE'], array('000', '012', '013')) === true){
				# 클레임상태 업데이트
				$query = "update ".INPK_CLAIM_ITEM." set clm_statnm='반품/교환입고확정전송', latedt=now(), storeDvno='{$data['deliveryno']}', storeDvcode='{$data['deliverycode']}' where clmsno='{$data['clmsno']}' and itmsno='{$data['itmsno']}'";
				$GLOBALS['db']->query($query);
				# 로그추가
				$query = "insert into ".INPK_CLAIM_ITEM_LOG." values ('', '{$data['itmsno']}', '반품/교환수거지시 > 반품/교환입고확정전송', now())";
				$GLOBALS['db']->query($query);
		...
		$this->log("clmStoreCompForComm END");
	...
	### 교환확정
	function exchangeCompForComm($data)
	{
		$this->callCnt++;
		$this->log("exchangeCompForComm START");
		...
		if ($gClaim['clm_seq'] == ''){
			...
			if (in_array($arrXml['ORDER_LIST']['RESULT']['CODE'], array('000', '014')) === true){
				# 클레임상태 업데이트
				$query = "update ".INPK_CLAIM_ITEM." set clm_statnm='교환확정전송', latedt=now(), exchangeDvno='{$data['deliveryno']}', exchangeDvcode='{$data['deliverycode']}' where clmsno='{$data['clmsno']}' and itmsno='{$data['itmsno']}'";
				$GLOBALS['db']->query($query);
				# 로그추가
				$query = "insert into ".INPK_CLAIM_ITEM_LOG." values ('', '{$data['itmsno']}', '교환/재배송출고지시 > 교환확정전송', now())";
				$GLOBALS['db']->query($query);
		...
		$this->log("exchangeCompForComm END");
