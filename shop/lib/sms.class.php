...
class Sms
{
	...
	function sms_socket($arr){
		$host = "godosms.godo.co.kr";
		$port = 1688;
		if ($this->godo['smsModule'] == 'l') { // LG
			$port = 1687;
		}
		foreach($arr as $k => $v) $strSms .= $k.":".$v."\n";
		if($strSms){
	...
	function send($tran_msg,$tran_phone,$tran_callback='',$send_date='',$alram_etc='',$tran_type='send')
	{
		if ( $this->smsPt <= 0 ){
			if ($this->msgOn) {
	...
	function sms_Control($phone,$msg,$alram_Date,$etc_data,$type){
		if( $type == "new" ){
			$this->send($msg,$phone,$phone,$alram_Date,$etc_data,'res_send');
			$this->update_ok_eNamoo = true;
			$this->update();
		}else if( $type == "delete" ){
			$this->send($msg,$phone,$phone,$alram_Date,$etc_data,'res_delete');
			$this->update_ok_eNamoo = true;
			$this->update();
	...
	*/
	...
	function setMsgProcess($_msg, $parseCode='')
	{
		...
		$_msg = gd_str_split($_msg, 90);
		$msg = addslashes($_msg[0]);
	...
	function getSmsShutdownLogMessage()
	{
		...
		$errorMessage['SMS_MESSAGE'] = $_POST['msg'];
		$errorMessage['SMS_SEND_TYPE'] = 'SMS';
