...
class i2e_order_api extends putLog
{
	...
	function i2e_order_api($isTest=false)
	{
		...
		switch ($this->interfaceid)
		{
			...
			case "orderListDelvForComm": # 주문내역조회2
				$this->orderListForComm($arrXml);
				break;

			case "cnclNClmNExchReqListForComm": # 주문취소/반품/교환요청리스트
				$this->cnclNClmNExchReqListForComm($arrXml);
				break;

			case "clmListForComm": # 클레임리스트
				$this->clmListForComm($arrXml);
				break;

			case "orderCompListForComm": # 구매확정조회
				$this->orderCompListForComm($arrXml);
				break;
		}
    ...
    ### 주문내역조회(Insert Order)
    function orderListForComm($arrXml)
    {
		...
		foreach($arrXml['ORDER_LIST']['ORDER'] as $order)
		{
			...
			if ($gOrder['ordno'] != ''){
				unset($msg);
				$msg[] = "Fail : Existent order of interpark [";
				...
				## 결제로그
				$settlelog =  sprintf("[%s] 인터파크샵플러스 입금확인된 주문\n", date('y-m-d H:i:s'));
			...
			foreach($order['PRODUCT']['PRD'] as $k => $item)
			{
				...
				if ($gOrderItem['sno'] != ''){
					unset($msg);
					$msg[] = "Fail : Existent orderItem of interpark [";
					...
					## 상품정보 가져오기
					$data = $GLOBALS['db']->fetch("select goodsno, goodsnm, maker, tax, brandno, usestock from ".GD_GOODS." where inpk_prdno='{$item['PRD_NO']}'");
					...
					if($data['usestock'] == 'o') $stockable = "y"; else $stockable = "n";
    ...
    ### 주문취소/반품/교환요청리스트
    function cnclNClmNExchReqListForComm($arrXml)
    {
		if (is_array($arrXml['ORDER_LIST']['ORDER']) === false){
    ...
    ### 클레임리스트
    function clmListForComm($arrXml)
    {
		if (is_array($arrXml['ORDER_LIST']['ORDER']) === false){
    ...
    ### 구매확정조회
    function orderCompListForComm($arrXml)
    {
		if (is_array($arrXml['ORDER_LIST']['ORDER']) === false){
		...
		foreach($arrXml['ORDER_LIST']['ORDER'] as $order)
		{
			...
			if ($gOrder['ordno'] == ''){
				unset($msg);
				$msg[] = "Fail : Non-existent order of interpark [";
				$msg[] = sprintf("\t인터파크 주문번호 = %s", $order['ORD_NO']);
				$msg[] = "]";
				$this->log($msg);
				continue;
			...
			foreach($order['PRODUCT']['PRD'] as $k => $item)
			{
				...
				if ($gOrderItem['sno'] == ''){
					unset($msg);
					$msg[] = "Fail : Non-existent orderItem of interpark [";
					$msg[] = sprintf("\t인터파크 상품코드 = %s", $item['PRD_NO']);
					$msg[] = sprintf("\t인터파크 주문순번 = %s", $item['ORD_SEQ']);
					$msg[] = "]";
					$this->log($msg);
