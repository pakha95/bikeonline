...
class cashreceipt
{
	...
	function getReceipturl($crno, $mode='')
	{
		...
		{
			$ordno = $crno;
			list($cashreceipt, $mobileOrder, $settlekind, $cdt, $settleprice) = $db->fetch("select cashreceipt, mobileOrder, settlekind, cdt, settleprice from ".GD_ORDER." where ordno='{$ordno}'");
			if ($cashreceipt != '') $where = "or (cashreceipt='{$cashreceipt}' and status='ACK')";
			$data = $db->fetch("select crno, pg, tid, certno, ordno, receiptnumber, moddt, amount from ".GD_CASHRECEIPT." where ordno='{$ordno}' {$where} order by crno desc limit 1");
			if ($data['crno'] == ''){
				...
				$data['moddt'] = $cdt;
				$data['amount'] = $settleprice;
			}
		}
		else {
		$data = $db->fetch("select crno, pg, tid, certno, ordno, receiptnumber, moddt, amount from ".GD_CASHRECEIPT." where crno='{$crno}'");
		}
		...
		if ($data['pg'] == 'allat' && $data['tid'] && $data['certno'])
		{
			if ($data['crno'] == '' && ($settlekind == 'o' || $settlekind == 'v'));
				...
				$url = 'https://www.allatpay.com/servlet/AllatBizPop/member/pop_cash_receipt.jsp?receipt_seq_no='.$data['tid'].'&shop_id='.$this->pg['id'].'&amt='.$data['amount'];
			}
		}
