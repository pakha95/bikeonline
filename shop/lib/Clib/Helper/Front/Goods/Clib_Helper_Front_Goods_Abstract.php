...
class Clib_Helper_Front_Goods_Abstract extends Clib_Helper_Abstract
{
	...
	public function prepareGoodsCollection($collection, $params)
	{
		...
		if (array_key_exists('mode', $params)) {// 0 이 허용되기 때문에 키가 있는지 체크함
			...
			if ($params['event_mode_type'] == 'group') {
				$collection->addFilter('goods_display.mode', $params['mode'], 'in');
			} else {
	...
	public function getCategoryModel($id = null, $goodsModel = null)
	{
		...
		if ($id) {
			...
			$categories->addFilter('hidden', 0);
					...
					// 모바일샵 카테고리 노출 설정이 '모바일샵 별도 노출설정 적용'인 경우
					$categories->addFilter('hidden_mobile', 0);
				}
			}
			else {
				$categories->addFilter('hidden', 0);
	...
	{
		global $cfg, $cfgCoupon, $Acecounter, $db, $snsCfg, $cfg_related, $set, $tpl;
		...
		//해외/국내배송 by jung
		$goods_prefix = $goodsModel[goods_prefix];
		// 회원정보 가져오기
		...
		for ($i = 0; $i < $m; $i++) {
			...
			if ($cfgCoupon['coupon'] && (is_file($tpl->template_dir.'/proc/popup_coupon_division.htm') || is_file($tpl->template_dir.'/proc/coupon_list_division.htm'))) {
				if ($realprice > $tmp['coupon']) {
					$tmp['couponprice'] = $realprice - $tmp['coupon'];
				} else {
					$tmp['couponprice'] = 0;
					$tmp['coupon'] = $realprice;
				}
			} else {
				$tmp['couponprice'] = $realprice - $tmp['coupon'];
			}
		...
		// 레코픽
		/*require_once SHOPROOT."/lib/recopick.class.php";
		if (class_exists('Recopick')) {
			$Recopick = new Recopick(Clib_Application::isMobile());
			if (Clib_Application::isMobile()) {
				$recopick_imgs = array(
					'use_mobile_img' => $goodsModel['use_mobile_img'],
					'img_y' => $goodsModel['img_y'],
					'img_pc_y' => $goodsModel['img_pc_y'],
					'img_l' => $goodsModel['img_l'],
					'img_m' => $goodsModel['img_m'],
					'img_s' => $goodsModel['img_s'],
					'img_i' => $goodsModel['img_i']
				);
				$customHeader .= $Recopick->get_metaTag_script($goodsModel['goodsnm'], $recopick_imgs, $goodsModel['price'],$goodsModel['brand']);
			}
			else {
				$systemHeadTagEnd .= $Recopick->get_metaTag_script($goodsModel['goodsnm'], $goodsModel['r_img'], $goodsModel['price'],$goodsModel['brand']);
			}
		}
		*/
		// 추가옵션
	...
	protected function setGoodsCollectionAttributes($collection, $category)
	{
		...
		foreach ($collection as $item) {
			...
			if (strlen($item['strprice']) > 0) { // 가격대체문구 있는 경우 초기화
				$item['coupon'] = '';
				$item['coupon_emoney'] = '';
			}
			else {
