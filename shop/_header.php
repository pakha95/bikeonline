...
{ // File Key
	...
	} else if($key_file == 'mypage/mydelivery.htm' && $_GET['htmid'] != 'mypage/mydelivery.htm' && isset($design_skin['mypage/mydelivery.htm']) === false) {
		$data_file['outline_side'] = 'outline/side/mypage.htm';
	}
}
...
### 성인인증 인트로
if($cfg[custom_landingpage] > 1 && !preg_match( "/main\/intro*/", $key_file) && !preg_match( "/member\/find*/", $key_file) && !preg_match( "/member\/join*/", $key_file) && !preg_match( "/member\/login*/", $key_file) && !preg_match( "/proc\/popup_zipcode*/", $key_file) && !preg_match( "/member\/change*/", $key_file) && !preg_match( "/proc\/popup_address*/", $key_file) && !preg_match( "/mypage\/mypage_delivery*/", $key_file)){
