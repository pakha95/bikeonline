...
switch ($_POST[mode]){
	...
	case "modMember":
		...
		if($db->query($query)){
			...
			### 배송지 목록 - 기본 배송지 동기화
			$delivery_sno = "SELECT dm.gmd_sno FROM ".GD_MEMBER_DELIVERY." dm WHERE dm.gmd_m_no = (SELECT m_no FROM ".GD_MEMBER." WHERE m_id='".$sess['m_id']."') AND dm.gmd_default = 'y' LIMIT 1";
