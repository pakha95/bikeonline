...
// settle페이지에 접근시에도 잔여 재고 체크추가
//if ($cart->item) {
//	foreach ($cart->item as $v) {
//		$cart->chkStock($v['goodsno'], $v['opt'][0], $v['opt'][1], $v['ea'], '', '', $v['sales_unit']);
//	}
//}
...
// 배송지 선택이 '신규 배송지'(3) 아니면 '배송지 목록에 추가'와 '기본 배송지로 선택' 초기화
if ((string)$_POST['deli_select'] !== '3') {
	unset($_POST['delivery_default']);
	unset($_POST['delivery_add']);
}
