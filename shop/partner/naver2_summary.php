...
while($row = $db->fetch($result,1))
{
	...
	if ($cfgCoupon['use_yn'] && $partner['uncoupon'] === 'N') {
		if($couponVersion === true) list($couponDiscount, $couponEmoney) = getCouponInfo($row['mapid'], $_row['price'],'','','all');
		else list($couponDiscount, $couponEmoney) = getCouponInfo($row['mapid'], $_row['price']);
		if ($couponDiscount) {
	...
	$coupon += 0;
	$dcprice += 0;
	if ($coupon > $_row['price'] - $dcprice && $couponVersion === true) $coupon = $_row['price'] - $dcprice;
