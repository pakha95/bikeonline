...
while($row = $db->fetch($result,1))
{
	...
	if($cfgCoupon['use_yn'] && $partner['uncoupon'] == 'N'){
		if($couponVersion ===  true) list($row['coupon'],$row['coupon_emoney']) = getCouponInfo($row['goodsno'],$row['price'],'','','all');
		else list($row['coupon'],$row['coupon_emoney']) = getCouponInfo($row['goodsno'],$row['price']);
	...
	$coupon += 0;
	$dcprice += 0;
	if ($coupon > $ar_data['price'] - $dcprice && $couponVersion === true) $coupon = $ar_data['price'] - $dcprice;
