...
if($data['settleInflow'] == 'payco'){
	...
	if ($data['cashreceipt_ectway'] == 'Y')
	{
		...
		$query = "select certno, amount from gd_cashreceipt where ordno='$_GET[ordno]' order by crno desc limit 0,1";
		$cash = $db->fetch($query,1);
		if( $data[settlekind] != 'o' ){
			$tpl->assign('certno',$cash[certno]);
			$tpl->assign('amount',$cash['amount']);
		}else{
...
### 세금계산서
@include "../conf/config.taxGuide.php";
if ( $set[tax][useyn] == 'y' ){
	$tmp = 0;
	...
	//이용안내문구
	$guide = parseCode(nl2br($cfgtaxGuide['guideWords']));
}
...
else if ( $data[taxapp] == 'Y' ) {
	$data[taxmode] = 'taxapp';
	//세금계산서 마감안내문구 노출
	$endDate = date('Y-m-10', strtotime(($data[cdt])."+1month"));
	if($set[tax][period] == "limit" && strtotime($endDate) < strtotime(date('Y-m-d'))){
		$data[words] = "end";
		$guide = parseCode(nl2br($cfgtaxGuide['endWords']));
	}
};
...
//고객상담메모 추가 시작
$tpl->assign('adminmemo',$data['adminmemo']);
//추가 끝
//배송사 추가 시작
$tpl->assign('dvNo',$data['dvno']);
$tpl->assign('dvCode',$data['dvcode']);
$tpl->assign('dvUrl',$data['deliveryurl']);
