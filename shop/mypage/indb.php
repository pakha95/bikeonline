...
switch ($_POST[mode]){
	...
	case "taxapp":
		...
		if($set['tax']['period'] == 'limit'){
			$endDate = date('Y-m-10', strtotime(($order['cdt']."+1month")));//마감일
			if(strtotime($endDate) < strtotime(date('Y-m-d'))){
				msg($msg="관리자에 의해 세금계산서 발행신청이 마감되었습니다. ". "\\n" . "세금계산서 관련 문의 사항은 고객센터로 문의 부탁드립니다.". "\\n" . "이용에 불편을 드려 죄송합니다." ,$code=$_SERVER[HTTP_REFERER]);
				exit;
			}
		}
		### 가격 계산
