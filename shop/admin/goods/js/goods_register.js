		...
		imageUpload : {
			toggleForm : function()
			{
				...
				if (m == 'file') {
					...
					$('image_attach_method_upload_wrap_help').style.display = 'block';
					$('image_attach_method_link_wrap').style.display = 'none';
					$('image_attach_method_link_wrap_help').style.display = 'none';
				}
				else {
					$('image_attach_method_upload_wrap').style.display = 'none';
					$('image_attach_method_upload_wrap_help').style.display = 'none';
					$('image_attach_method_link_wrap').style.display = 'block';
					$('image_attach_method_link_wrap_help').style.display = 'block';
				}
			...
			toggleImg : function()
			{
				...
				var method, method_obj = document.fm.image_attach_method;

				for (var i = 0; i < obj.length; i++) {
					if (obj[i].checked)
						var m = obj[i].value;
				}
				for (var i = 0; i < method_obj.length; i++) {
					if (method_obj[i].checked)
						var method = method_obj[i].value;
				}
				if (m == 1 && method == 'file') {
					jQuery('.image_file').show();
					jQuery('.use_mobile_img_1').show();
					jQuery('.use_mobile_img_0').hide();
					jQuery('.image_url').hide();
				}
				else if (m == 1 && method == 'url') {
					jQuery('.image_file').hide();
					jQuery('.image_url').show();
				}
				else {
					jQuery('.image_file').show();
