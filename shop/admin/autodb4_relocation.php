...
if($_GET['mode']=='insert'){
	...
	*/
	...
	/*	--------------------------------------------
	*	- gd_goods_option goodsno = 0 체크
	*	--------------------------------------------
	*/

	$goodsOptionQuery = "Select count(sno) cnt From gd_goods_option Where goodsno = 0;";
	$goodsOptionResult = $db->query($goodsOptionQuery);
	list($goodsOptionCount) = $db->fetch($goodsOptionResult);
...
?>
<tr>
	<td width="110" nowrap>상품 번호 0 옵션 삭제</td>
	<td width="100%">
		<table width="100%" border="0" cellspacing="0" cellpadding="3">
		<tr>
			<td>
				<input type="radio" name="gd_goods_option_del" value="y" checked />삭제&nbsp;
				<input type="radio" name="gd_goods_option_del" value="n" />삭제 안함&nbsp;
			</td>
		</tr>
		</table>
	</td>
...
<tr>
	...
	<td width="100%">
		...
		<tr>
			<td>
				<input type="radio" name="gd_goods_longdesc" value="y"  />상품 상세 내용 이미지 경로 업데이트&nbsp;
				<input type="radio" name="gd_goods_longdesc" value="n"checked />업데이트 안함&nbsp;
...
<?php
	...
	// 상품 옵션 0 삭제
	if ($_POST['gd_goods_option_del'] == 'y') {
		$goodsOptionDeleteTF = true;

		$goodsOptionDeleteQuery = "Delete From gd_goods_option Where goodsno = 0";
		if (!$db->query($goodsOptionDeleteQuery)) {
			$goodsOptionDeleteTF = false;
		}
	}
	...
	if ($_POST['gd_option_upyn'] == 'y') {
		...
		$goodsPriceUpdateQuery = "UPDATE gd_goods gg 
									JOIN (SELECT gsno.goodsno, 
												price, 
												consumer, 
												supply, 
												reserve 
											FROM	gd_goods_option go 
												JOIN (SELECT sno, 
																goodsno 
														FROM		(SELECT goodsno, 
																		Count(sno) cnt, 
															  ...
															  GROUP	BY goodsno 
															  ORDER	BY sno) A) gsno 
													ON go.goodsno = gsno.goodsno 
											WHERE	link = 1 
												AND go.sno = gsno.sno) ggo 
										ON gg.goodsno = ggo.goodsno 
							SET		gg.goods_price = ggo.price, 
									gg.goods_supply = ggo.supply, 
									gg.goods_consumer = ggo.consumer, 
									gg.goods_reserve = ggo.reserve;
								";
...
<tr>
	<td width="100%">
		<?=($goodsOptionDeleteTF) ? '<font color="#0000CC">gd_goods_option 상품번호 0 옵션 삭제 <br>' . $goodsOptionDeleteQuery . '</font>' : '<font color="#CC0000">gd_goods_option 상품번호 0 옵션 삭제 실패<br>' . $goodsOptionDeleteQuery . '</font>'?>
	</td>
</td>
<?php
	}
