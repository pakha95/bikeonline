...
//sms버튼추가용 by jung
function sendSMS(){
	sendSmsCase('delivery',$order[mobileOrder]);
	echo $order[mobileOrder];
}
//끝
...
<tr bgcolor="<?=$bgcolor?>" bg="<?=$bgcolor?>">
	...
	<?if($set[delivery][basis]){?>
	<td><div nowrap><font class=small color=555555><b><?=((!$item[dvcode]||!$item[dvno])&&$order[step])? "-":$r_delivery[$item[dvno]] . "</div><div nowrap>" . $item[dvcode]?></font><? if($item[dvcode]){?><p><a href="javascript:popup('../member/popup.sms.php?dvmaker=<?=$item[maker]?>&dvcomp=<?=$r_delivery[$item[dvno]]?>&dvcode=<?=$item[dvcode]?>&mobile=<?=$order['mobileOrder']?>',780,600)"><img src="../img/btn_sms.gif" align=absmiddle></a></p><?}?></div></td>
	<?}?>
	<td width=70 nowrap>
...
<tr>
	...
	<td nowrap><font class=ver8 color=444444>
		...
		if($row[dc]){
			...
			if($row['dc'] != $row['realdc']) echo " (실적용금액:".number_format($row['realdc'])."원)";
		}
		if($row[emoney]){
