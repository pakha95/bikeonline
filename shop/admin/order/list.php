...
if($isEnableAdminCheckoutOrder !== true) {
	...
	$query = '
		...
		from
			...
			left join '.GD_GOODS.' as g on g.goodsno = oi.goodsno
			'.$join_GD_COUPON_ORDER.'
		'.$strWhere.'
		group by o.ordno
	';
...
<tr>
	...
	<option value="2">해외주문/배송준비중 처리</option>
...
<?php
	...
	foreach($eachOrderGroup as $eachOrder):
		if($eachOrder['count_item']>1) $goodsnm = $eachOrder['goods_prefix'].$eachOrder['goodsnm'].' 외'.($eachOrder['count_item']-1).'건';
		else $goodsnm = $eachOrder['goods_prefix'].$eachOrder['goodsnm'];
...
<tr>
	<td style="text-align: left; padding: 3px 0px 3px 0px;">선택한 주문건을 : <img src="../img/orderSmsSendSmall.png" alt="주문자에게 SMS발송" border="0" class="hand" onclick="javascript:openOrderSmsPopup(document.frmList, 'chk[]');" align="absmiddle" /></td>
</tr>
</table>

<?php include '../proc/_orderSmsSendInfo.php'; ?>
...
<tr><td><img src="../img/icon_list.gif" align="absmiddle"/>주문상태변경을 통해 각 주문처리단계 (주문접수, 해외배송준비중, 해외배송중, 국내배송중, 배송완료) 로 빠르게  처리하실 수 있습니다.</td></tr>
