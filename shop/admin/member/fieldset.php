...
if($checked['reqField']['sms'] == "checked" || $checked['reqField']['mailling'] == "checked"){
	$checked['reqField']['sms'] = '';
	$checked['reqField']['mailling'] = '';
}
?>
...
<tr>
	<? $idx=0; while (list($key,$value)=each($fld['per'])){
			$dis = "";
			if(in_array($key, array('sms', 'mailling'))){
				$dis = "disabled";
			}
	?>
	...
	<td class="noline" width=130px><font class="def"><input type="checkbox" id="chkReq[]" name="reqField[<?=$key?>]" <?=$checked['reqField'][$key]?> key="<?=$key?>" onClick="chk('<?=$key?>','req');" <?=$dis?>/> 필수</td>
	...
	<td class="noline" width=130px><font class="def"><input type="checkbox" id="chkMobileReq[]" name="reqMobileField[<?=$key?>]" <?=$checked_mobile['reqField'][$key]?> key="<?=$key?>" onClick="chk_mobile('<?=$key?>','req');" <?=$dis?>/> 필수</td>
	</tr><tr>
	<? } ?>
</tr>
</table>
