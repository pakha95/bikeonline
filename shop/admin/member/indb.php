...
switch ($mode){
	...
	case "send_sms":
		...
		switch ($_POST[type]){
			...
			case "9": //SMS 발송결과 재전송 - 검색
				...
				$total	= $db->count_($res);
				break;
			case "10": //주문자에게 SMS 발송
				$to_tran= $r_smsType[$_POST['type']];
				if($_POST['includeFail'] == 'N') $where .= " AND o.mobileOrder NOT IN ('".implode("','", $failListArr)."') ";
				if($_POST['receiveRefuseType'] == 'N') $where .= " AND o.m_no > 0 AND m.sms = 'y' "; // 수신거부 제외 N - 제외, Y or '' - 포함

				$query	= "SELECT o.mobileOrder, o.nameOrder, m.m_no FROM ".GD_ORDER." AS o LEFT JOIN ".GD_MEMBER." as m ON o.m_no=m.m_no WHERE o.ordno IN (".implode(",", $_POST['chk']).")".$where. " GROUP BY o.mobileOrder";
				$res	= $db->query($query);
				$total	= $db->count_($res);
				break;
		}
		...
		if($_POST['sms_type'] == 'sms'){
			...
			$smspt = number_format($sms->smsPt, 1);
		...
		// LMS
			...
			$smspt = number_format($lms->smsPt, 1);
	...
	case "modify":
		...
		### 배송지 목록 - 기본 배송지 동기화
		$delivery_sno = "SELECT dm.gmd_sno FROM ".GD_MEMBER_DELIVERY." dm WHERE dm.gmd_m_no = (SELECT m_no FROM ".GD_MEMBER." WHERE m_id='".$m_id."') AND dm.gmd_default = 'y' LIMIT 1";
		list ($gmd_sno) = $db->fetch($delivery_sno);
		if($gmd_sno) {
			$delivery_update = "
			UPDATE ".GD_MEMBER_DELIVERY." SET
				gmd_zipcode = '".$_POST['zipcode'][0]."-".$_POST['zipcode'][1]."',
				gmd_address = '".$_POST['address']."',
				".($_POST['road_address'] ? "gmd_road_address = '".$_POST['road_address']."'," : '')."
				".($_POST['address_sub'] ? "gmd_address_sub = '".$_POST['address_sub']."'," : '')."
				gmd_phone = '".$_POST['phone'][0]."-".$_POST['phone'][1]."-".$_POST['phone'][2]."',
				gmd_mobile = '".$_POST['mobile'][0]."-".$_POST['mobile'][1]."-".$_POST['mobile'][2]."',
				".($_POST['zonecode'] ? "gmd_zonecode = '".$_POST['zonecode']."'," : '')."
				gmd_moddate = NOW()
			WHERE gmd_sno = ".$gmd_sno;
			$db->query($delivery_update);
		}
