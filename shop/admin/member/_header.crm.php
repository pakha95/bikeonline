<?
$crm_view = true;
$member_query = "SELECT m_no, m_id, name FROM ".GD_MEMBER." WHERE (m_id IS NOT NULL AND m_id<>'')";
if ($_GET['m_no']) $member_query .= " AND m_no='".$_GET['m_no']."'";
if ($_GET['m_id']) $member_query .= " AND m_id='".$_GET['m_id']."'";
$member_query .= " LIMIT 1";
list($m_no,$m_id,$name) = $db->fetch($member_query);
