...
$smsFailCnt = 0;
$dvmaker = $_REQUEST['dvmaker'];
$dvcomp = $_REQUEST['dvcomp'];
$dvcode = $_REQUEST['dvcode'];
$dvMSG = "[바이크온라인] ".$dvmaker."상품이 ".$dvcomp." ".$dvcode."으로 배송중입니다.";
switch ($_POST[type]){
	case "1":				// 개별 발송
	...
	case "10": //주문자에게 SMS 발송
		$to_tran= "주문자에게 SMS 발송";
		if(is_array($_POST['chk']['enamoo'])){
			$_POST['chk'] = $_POST['chk']['enamoo'];
		}
		$query	= "SELECT mobileOrder FROM ".GD_ORDER." WHERE mobileOrder AND ordno IN (".implode(",", $_POST['chk']).") GROUP BY mobileOrder";
		$res	= $db->query($query);
		$total	= $db->count_($res);
		...
		$resCnt = $db->query($query);
		while($row = $db->fetch($resCnt, 1)){
			$phoneNumberArr[] = $row['mobileOrder'];
		}
		$failSnoArr = smsFailCheck('array', $phoneNumberArr);
		break;
}
...
//수신거부목록 확인 [회원 주소록 검색목록, 회원 주소록 선택 목록, 주문자에게 SMS 발송]
$smsReceiveRefuseCount = 0;
if(in_array($_POST[type], array('2', '3', '10'))){
	$receiveRefuseQuery = '';
	switch($_POST[type]){
		case '2' :
		...
		case '10' :
			$receiveRefuseQuery	= "SELECT o.mobileOrder FROM ".GD_ORDER." AS o LEFT JOIN ".GD_MEMBER." as m ON o.m_no=m.m_no WHERE o.mobileOrder AND o.ordno IN (".implode(",", $_POST['chk']).") AND (o.m_no < 1 OR m.sms = 'n' OR !m.m_no) GROUP BY o.mobileOrder";
			$res = $db->query($receiveRefuseQuery);
			$smsReceiveRefuseCount = $db->count_($res);
		break;
	}
