...
// SMS 발송 가능 건수
if ($total > $sms->smsPt){
	msg("SMS 발송예정인 ".number_format($total)."건이 잔여콜수인 ".number_format($sms->smsPt, 1)."건보다 많습니다");
	exit;
}
...
msg($msg);
echo "<script>parent.document.getElementById('span_sms').innerHTML = '".number_format($sms->smsPt, 1)."'; parent.document.getElementById('sms_bar').style.width = 0;</script>";
