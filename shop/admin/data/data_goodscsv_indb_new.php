...
if ( empty( $_FILES['file_excel'][name] ) ) $altMsg = 'CSV파일을 선택하지 않으셨습니다.'; // 화일이 없으면
	...
	foreach ($csv as $row) {
		...
		if (sizeof($_gd_goods) > 0) {
			...
			if ( $_gd_goods['goods_delivery'] && in_array($_gd_goods['delivery_type'], array('0', '1'))) $_gd_goods['goods_delivery'] = ''; // 기본, 무료배송 일 경우 배송비

			// 상품정보 저장
			$goodsno = $db->procedure( 'save_goods', $_gd_goods );

			// 옵션 저장
			if ($goodsno) { // 옵션이 없어도 기본 1개는 등록되어야 함
			...
			// 카테고리
			if ($goodsno && sizeof($_gd_goods_link) > 0) {
		...
		fwrite($tmp_fp,  '<td>처리결과</td><td>INSERT (' . ( $goodsno ? 'T' : 'F' ) . ')' . '</td>');
		fwrite($tmp_fp,  '</tr>');
