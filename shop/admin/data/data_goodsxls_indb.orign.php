				...
				if ( $_GET['sample'] != 'Y' ) {
					...
					while ( $data = $db->fetch($res) ) {
						...
						foreach ( $fields as $key => $arr ) {
							...
							} else if ( $key == 'longdesc'  || $key == 'goodsnm' || $key == 'goodscd' || $key == 'origin' || $key == 'maker' || $key == 'keyword' || $key == 'strprice' || $key == 'shortdesc' || $key == 'memo' || $key == 'addoptnm' || $key == 'ex_title' || $key == 'ex1' || $key == 'ex2' || $key == 'ex3' || $key == 'ex4' || $key == 'ex5' || $key == 'ex6') {
