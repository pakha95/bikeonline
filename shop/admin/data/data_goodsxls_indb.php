					...
					while ( $data = $db->fetch($res) ) {
						...
						foreach ( $fields as $key => $arr ) {
							...
							if ( $key == 'goodscate' ) {
								...
								$tmp = array();
								$t_res = $db->query( "select opt1, opt2 from ".GD_GOODS_OPTION." where goodsno = '" . $data['goodsno'] . "' AND go_is_deleted <> '1' order by link desc, sno" );
								while ( $t_row = $db->fetch( $t_res, "MYSQL_ASSOC" ) ) if($t_res['opt1']&&$t_res['opt2']) {$tmp[] = implode( ";", $t_row );}
								...
								foreach ( $_addoptnm as $step => $recode ){
									...
									if ($_tmp[2] == 'I') {
										...
										if($_tmp[0]&&$_tmp[1]){$addoptnm[] = $_tmp[0] . ';' . $_tmp[1];}
									}
								}
