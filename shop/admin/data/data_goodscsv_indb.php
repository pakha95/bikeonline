		...
		while ( $data = fgetcsv( $fp, 135000, ',' ) ){ // 데이타 저장
			//if($data[$FieldNm[goodsno]]){ //널체크 by birdmarine 2007.1.30
				...
				foreach ( $FieldNm as $key => $dataIdx ){ // Recode 배열 구성
					...
					//모바일이미지 기본적으로 미사용 by jung
					else if ( !isset($Recode['use_mobile_img']) ) $Recode['use_mobile_img'] = '0';
				...
				{ // Recode 배열 저장
					...
					if ($insert == true && !array_key_exists('regdt',$Recode)) {
						$tmpSQL[] = "regdt = NOW()";
					}

					// 옵션 사용 여부 (옵션 등록 처리후 다시 갱신)
					////if ($getScnt == 0 || ($getScnt != 0 && array_key_exists('opts',$Recode))) {
					if ($insert == true || ($insert == false && array_key_exists('opts',$Recode))) {
      					$tmpSQL[] = "use_option='0'";
     				}
				...
				if ($insert == false && !array_key_exists('opts', $etcRecode)) {
					$etcRecode['opts'] = '';
				}
				$price = '';
				...
				{ // 결과출력
					...
					fwrite($tmp_fp,  '<td>처리결과</td><td>' . ( $insert == true || $getScnt == 0 ? 'INSERT' : 'UPDATE' ) . ' (' . ( $result1 ? 'T' : 'F' ) . ')' . '</td>');
					fwrite($tmp_fp,  '</tr>');
