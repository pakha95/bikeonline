...
<div class=title>구매자 거래명세표 출력 설정 <span>구매자의 거래명세표 출력 여부 및 단계를 설정합니다.</span> <a href="javascript:manual('<?=$guideUrl?>board/view.php?id=basic&no=17')"><img src="../img/btn_q.gif" border=0 align=absmiddle></a></div>
<table class=tb>
<col class=cellC><col class=cellL>
<tr>
	<td>출력 사용 여부</td>
	<td class="noline">
		<label><input type="radio" name="particular" value="y" <?=($cfg[particular] == 'y') ? 'checked' : ''?> onclick="particularApper();">사용함</label>
		<label><input type="radio" name="particular" value="n" <?=($cfg[particular] != 'y') ? 'checked' : ''?> onclick="particularApper();">사용안함</label>
		<div class="extext" style="padding: 5px 0 0 3px;line-height:140%;">
		사용으로 설정 시 쇼핑몰의 주문내역상세보기 페이지에서 거래명세표 출력이 가능합니다.
		</div>
	</td>
</tr>
<tr class="particularApperTarget" style="display:'none';">
	<td>출력 허용 단계</td>
	<td class="noline">
		<label><input type="radio" name="particularPermit" value="0" <?=($cfg[particularPermit] == '0' || !$cfg[particularPermit]) ? 'checked' : ''?>>주문접수</label>
		<label><input type="radio" name="particularPermit" value="1" <?=($cfg[particularPermit] == '1') ? 'checked' : ''?>>입금확인</label>
		<label><input type="radio" name="particularPermit" value="2" <?=($cfg[particularPermit] == '2') ? 'checked' : ''?>>배송준비중</label>
		<label><input type="radio" name="particularPermit" value="3" <?=($cfg[particularPermit] == '3') ? 'checked' : ''?>>배송중</label>
		<label><input type="radio" name="particularPermit" value="4" <?=($cfg[particularPermit] == '4') ? 'checked' : ''?>>배송완료</label>
	</td>
</tr>
<tr class="particularApperTarget" style="display:none;">
	<td>인감이미지</td>
	<td class="noline">
		<div class="extext" style="padding: 5px 0 0 3px;line-height:140%;">
		기본관리 > 세금계산서 설정에서 설정된 인감이미지를 사용합니다.
		</div>
	</td>
</tr>
</table>
...
function particularApper(){
	if (document.getElementsByName('particular')[0].checked) {
		document.getElementsByClassName('particularApperTarget')[0].style.display = "table-row";
		document.getElementsByClassName('particularApperTarget')[1].style.display = "table-row";
	}
	else {
		document.getElementsByClassName('particularApperTarget')[0].style.display = "none";
		document.getElementsByClassName('particularApperTarget')[1].style.display = "none";
	}
}
</script>
