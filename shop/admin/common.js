...
//주문리스트 SMS 발송 팝업
function openOrderSmsPopup(f, checkboxName)
{
	if(!f) return;
	...
	if(checkBoxChecked === false){
		...
		alert("네이버 페이와 페이코 간편구매 주문은 이나무의 주문을 거치지 않으므르 주문자의 휴대폰번호가 없어 제외하고 발송합니다.");
	}

	//input type 설정
	var originalTypeValue = '';
	var inputHiddenTypeExists = false;
	if(typeof f.type == 'undefined'){
		var inputTypeElement = document.createElement("input");
		inputTypeElement.type = 'hidden';
		inputTypeElement.name = 'type';
		inputTypeElement.value = 10;
		f.appendChild(inputTypeElement);
	}
	else {
		inputHiddenTypeExists = true;
		originalTypeValue = f.type.value;
		inputTypeElement = f.type;
		inputTypeElement.value = 10;
	}
	...
	//input type이 기존 존재
	if(inputHiddenTypeExists === true){
		f.type.value = originalTypeValue;
	}
	else {
		if(typeof inputTypeElement != 'undefined') {
			inputTypeElement.parentNode.removeChild(inputTypeElement);
		}
	}
}
