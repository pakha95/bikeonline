...
while($data=$db->fetch($res)) {
	list($data['m_id'], $data['sex']) = $db->fetch("SELECT m_id, sex FROM ".GD_MEMBER." WHERE m_no = '".$data['m_no']."'");
	if($data['titleStyle']) {
...
<input type="hidden" name="proc_board[<?=$data["no"]?>]" value="<?=$data['board']?>">
	...
	<td>
	<? if($data['m_id']) { ?>
		<span id="navig" name="navig" m_id="<?=$data['m_id']?>" m_no="<?=$data['m_no']?>"><font class="small1" color="#0074ba"><b><?=$data['name']?></b></font></span> <a href="javascript:popupLayer('../member/Crm_view.php?m_id=<?=$data['m_id']?>',780,600);"><img src="../img/icon_crmlist<?=$data['sex']?>.gif" /></a>
	<? }else{ ?>
		<font class="small1" color="#0074ba"><b><?=$data['name']?></b></font>
	<? } ?>
	</td>
	<td><?=$data['regdt']?></td>
	<td><?=$data['hit']?></td>
