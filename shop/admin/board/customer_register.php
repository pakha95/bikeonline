...
$data = $db->fetch("SELECT C.*, MB.m_id FROM ".$db_table." AS C LEFT JOIN ".GD_MEMBER." AS MB ON C.m_no = MB.m_no where C.sno='" . $_GET['sno'] . "'", 1);
$checked['secret'] = "";
...
<form name=form method=post action="<?=$sitelink->link('admin/board/customer_indb.php','ssl');?>" onsubmit="return chkEncoding(this)">
...
<? if($data['sno'] == $data['parent'] && $mode == "reviewReply") { ?>
	<tr>
		<td>적립금지급이유</td>
		<td>
...
?>
	...
	<? if($data['mobile'] != "--" && $data['mobile']) { ?>
	<tr>
		<td>SMS(카카오 알림톡)전송</td>
	<td><label class="noline"><input type="checkbox" name="smsyn" value="Y" <?=$disabled['sms']?> /> 고객에게 답변SMS(카카오 알림톡)을 동시에 전송하시겠습니까?</label></td>
	</tr>
	<? } ?>
<? } ?>
