<?
	...
	if( $_GET['board'] != 'all'){
		$tables[] = "( SELECT '".$board[$_GET['board']]."' AS boardnm, '".$_GET['board']."' AS board, b.no, b.titleStyle, b.subject, b.name, b.m_no, b.main, b.comment, b.regdt, b.hit, b.idx, HEX(b.sub) AS sub, m.m_id, m.mobile, m.email, b.ip, b.contents, b.old_file FROM gd_bd_".$_GET['board']." AS b LEFT JOIN ".GD_MEMBER." AS m ON b.m_no = m.m_no";
		...
		foreach($board as $key => $val) {
			$t_cnt++;
			$tables[] = " SELECT '".$val."' AS boardnm, '".$key."' AS board, b.no, b.titleStyle, b.subject, b.name, b.m_no, b.main, b.comment, b.regdt, b.hit, b.idx, HEX(b.sub) AS sub, m.m_id, m.mobile, m.email, b.ip, b.contents, b.old_file ";
