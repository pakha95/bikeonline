...
<?php }?>
	<tr>
		<th>받으실분</th>
		<td>
			<input type="text" name="nameReceiver" value="<?php echo $TPL_VAR["name"]?>" required class="max_width clear readonlyCheck" fld_esssential label="받으실분" />
		</td>
	</tr>
	<tr>
		<th>우편번호</th>
		<td class="zipcode">
			...
			<button id="zipcode-btn" class="btn_zipcode" type="button" onclick="frmMake('<?php echo $GLOBALS["cfg"]["rootDir"]?>/proc/popup_address.php?isMobile=true&gubun=mobile','searchZipcode','',false)" <?php if($GLOBALS["sess"]){?>style="display:none;"<?php }?>>우편번호</button>
			</div>
		</td>
	</tr>
	<tr>
	...
	<tr>
		<th>주소</th>
		<td>
			<div><input type="text" name="address" id="address" value="<?php echo $TPL_VAR["address"]?>" readonly required class="max_width clear readonlyCheck" onFocus="search_zipcode();this.blur();" fld_esssential label="주소" /></div>
		</td>
	</tr>
	<tr>
		<th>세부주소</th>
		<td>
			<input type="text" name="address_sub" id="address_sub" value="<?php echo $TPL_VAR["address_sub"]?>"  label="세부주소" class="max_width clear readonlyCheck" label="세부주소" onkeyup="SameAddressSub(this)" oninput="SameAddressSub(this)"/>
	...
	<tr>
		<th>휴대폰</th>
		<td class="phone">
			<input type="number" name="mobileReceiver[]" value="<?php echo $TPL_VAR["mobile"][ 0]?>" pattern="[0-9]*" min="000" max="999" maxlength="3" required fld_esssential label="받으실분 휴대폰번호" class="clear readonlyCheck" /> -
			<input type="number" name="mobileReceiver[]" value="<?php echo $TPL_VAR["mobile"][ 1]?>" pattern="[0-9]*" min="0000" max="9999" maxlength="4" required fld_esssential label="받으실분 휴대폰번호" class="clear readonlyCheck" /> -
			<input type="number" name="mobileReceiver[]" value="<?php echo $TPL_VAR["mobile"][ 2]?>" pattern="[0-9]*" min="0000" max="9999" maxlength="4" required fld_esssential label="받으실분 휴대폰번호" class="clear readonlyCheck" />
		</td>
	</tr>
	<tr>
		<th>메시지</th>
...
<table>
	<tr>
		<th>구매내용확인</th>
		<td class="noline">
			<input type="checkbox" name="doubleCheck" id="doubleCheck" value="y" required fld_esssential label="구매내용확인" msgR="구매 내용에 동의하셔야 결제가 가능합니다." />
...
$(function () {
	$('#doubleCheck').click(function () {
            if ($('#doubleCheck').val() != undefined) {
				var msg1 = '해외주문 특성상 마감후 해외에 주문이 들어간 후에는 <br>무료 철회가 불가능하며 ';
				var msg2 = '<span style="color:red;font-weight:bold">소정의 수수료 (25%)</span> ';
				var msg3 = '가 <br>고객님께 부과되니 신중히 주문해주시기 바랍니다.<br>국내 재고 상품인 경우 왕복 택배비를 부담하시면<br> 수수료 없이 청약철회 및 교환이 가능합니다.';
