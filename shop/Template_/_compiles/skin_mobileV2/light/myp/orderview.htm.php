...
<?php if($GLOBALS["set"]["delivery"]["basis"]&&$TPL_V1["dvcode"]){?>
							<div class="btn_track_area inblock">
								<img src="/shop/data/skin_mobileV2/light/common/img/myp/06_btn_01.png" class="btn_track" href="javascript:" onclick="frmMake('<?php echo $GLOBALS["cfg"]["rootDir"]?>/mypage/mypage_delivery.php?item_sno=<?php echo $TPL_V1["sno"]?>','frmDelivery','배송조회',false)">
							</div>
<?php }elseif(!$GLOBALS["set"]["delivery"]["basis"]&&$TPL_VAR["deliverycode"]){?>
							<div class="btn_track_area inblock">
								<img src="/shop/data/skin_mobileV2/light/common/img/myp/06_btn_01.png" class="btn_track" href="javascript:" onclick="frmMake('<?php echo $GLOBALS["cfg"]["rootDir"]?>/mypage/mypage_delivery.php?ordno=<?php echo $TPL_V1["ordno"]?>','frmDelivery','배송조회',false)">
							</div>
<?php }?>
						<div class="btn_track_area inblock" >
...
<?php }?>
	...
	<div class="info">
		...
		<tr>
			<th>우편번호</th>		<td><?php echo $TPL_VAR["zonecode"]?> (<?php echo $TPL_VAR["zipcode"]?>)</td>
		</tr>
		<tr>
...
<?php }?>
		...
		<tr>
			<th>총주문금액</th>
			<td><span id="paper_goodsprice"><?php echo number_format($TPL_VAR["goodsprice"])?></span>원 <?php if($TPL_VAR["diffPrice"]){?><font color=red></span>(상품조정금액 <?php echo number_format($TPL_VAR["diffPrice"])?>원)</font><?php }?></td>
		</tr>
		<tr>
			<th>회원할인</th>
...
<?php if($TPL_VAR["enuri"]){?>
		<tr>
			<th>에누리</th>
			<td><?php echo number_format($TPL_VAR["enuri"]* - 1)?>원</td>
		</tr>
<?php }?>
