...
<tr>
	<td><img src="/shop/data/skin/damoashop/img/common/title_order_01.gif" border=0></td>
</tr>
<TR>
...
<div><img src="/shop/data/skin/damoashop/img/common/order_txt_01.gif" border=0></div>
<?php echo $this->define('tpl_include_file_1',"proc/orderitem.htm")?> <?php $this->print_("tpl_include_file_1",$TPL_SCP,1);?>


<?php if(!$GLOBALS["sess"]){?>
<!-- 비회원 개인정보 취급방침 내용 -->
<div style="margin-top:20;"><img src="/shop/data/skin/damoashop/img/common/order_txt_non.gif" border=0></div>
...
<div style="margin-top:20;"><img src="/shop/data/skin/damoashop/img/common/order_txt_02.gif" border=0></div>
...
<tr>
	...
	<tr>
		<td>주문하시는분</td>
		<td><input type=text name=nameOrder value="<?php echo $TPL_VAR["name"]?>" style="font-weight:bold" <?php echo $GLOBALS["style_member"]?> required msgR="주문하시는분의 이름을 적어주세요"></td>
	</tr>
<?php if($GLOBALS["sess"]){?>
	<tr>
		<td>주소</td>
...
<?php }?>
	<tr>
		<td>전화번호</td>
		<td>
		<input type=text name=phoneOrder[] value="<?php echo $TPL_VAR["phone"][ 0]?>" size=3 maxlength=3 option=regNum required label="주문자 전화번호"> -
		<input type=text name=phoneOrder[] value="<?php echo $TPL_VAR["phone"][ 1]?>" size=4 maxlength=4 option=regNum required label="주문자 전화번호"> -
		<input type=text name=phoneOrder[] value="<?php echo $TPL_VAR["phone"][ 2]?>" size=4 maxlength=4 option=regNum required label="주문자 전화번호">
		</td>
	</tr>
	<tr>
		<td>핸드폰번호</td>
		<td>
		<input type=text name=mobileOrder[] value="<?php echo $TPL_VAR["mobile"][ 0]?>" size=3 maxlength=3 option=regNum required label="주문자 핸드폰번호"> -
		<input type=text name=mobileOrder[] value="<?php echo $TPL_VAR["mobile"][ 1]?>" size=4 maxlength=4 option=regNum required label="주문자 핸드폰번호"> -
		<input type=text name=mobileOrder[] value="<?php echo $TPL_VAR["mobile"][ 2]?>" size=4 maxlength=4 option=regNum required label="주문자 핸드폰번호">
		</td>
	</tr>
	<tr>
		<td>이메일</td>
...
<?php if($GLOBALS["sess"]){?>
	<tr>
		...
		<td class="noline">
			...
			<a href="javascript:popup('./order_delivery.php?mode=list&pkind=p',700,550)"><img src="/shop/data/skin/damoashop/img/common/btn_delivery_list.png" title="배송지 목록" alt="배송지 목록" style="vertical-align:bottom" /></a>
		</td>
	</tr>
<?php }else{?>
	<tr>
...
<?php }?>
	<tr>
		<td>받으실분</td>
		<td><input type=text name=nameReceiver value="<?php echo $TPL_VAR["name"]?>" required readonlyCheck></td>
	</tr>
	<tr>
		<td>받으실곳</td>
		<td>
		<input type=text name="zonecode" id="zonecode" size="5" class="line" readonly value="<?php echo $TPL_VAR["zonecode"]?>" readonlyCheck />
		( <input type=text name=zipcode[] id="zipcode0" size=3 class=line readonly value="<?php echo $TPL_VAR["zipcode"][ 0]?>" required readonlyCheck> -
		<input type=text name=zipcode[] id="zipcode1" size=3 class=line readonly value="<?php echo $TPL_VAR["zipcode"][ 1]?>" required readonlyCheck> )
		<a href="javascript:popup('../proc/popup_address.php',500,432);" id="addressSearch" <?php if($GLOBALS["sess"]){?>style="display:none;"<?php }?>><img src="/shop/data/skin/damoashop/img/common/btn_zipcode.gif" align=absmiddle></a>
		</td>
	</tr>
	<tr>
		<td></td>
		<td><input type=text name=address id="address" class=lineBig readonly value="<?php echo $TPL_VAR["address"]?>" required readonlyCheck></td>
	</tr>
	<tr>
		<td></td>
		<td>
		<input type=text name=address_sub id="address_sub" class=lineBig value="<?php echo $TPL_VAR["address_sub"]?>" onkeyup="SameAddressSub(this)" oninput="SameAddressSub(this)" label="세부주소" readonlyCheck>
	...
	<tr>
		<td>전화번호</td>
		<td>
		<input type=text name=phoneReceiver[] value="<?php echo $TPL_VAR["phone"][ 0]?>" size=3 maxlength=3 option=regNum required label="수령자 전화번호" readonlyCheck> -
		<input type=text name=phoneReceiver[] value="<?php echo $TPL_VAR["phone"][ 1]?>" size=4 maxlength=4 option=regNum required label="수령자 전화번호" readonlyCheck> -
		<input type=text name=phoneReceiver[] value="<?php echo $TPL_VAR["phone"][ 2]?>" size=4 maxlength=4 option=regNum required label="수령자 전화번호" readonlyCheck>
		</td>
	</tr>
	<tr>
		<td>핸드폰번호</td>
		<td>
		<input type=text name=mobileReceiver[] value="<?php echo $TPL_VAR["mobile"][ 0]?>" size=3 maxlength=3 option=regNum required label="수령자 핸드폰번호" readonlyCheck> -
		<input type=text name=mobileReceiver[] value="<?php echo $TPL_VAR["mobile"][ 1]?>" size=4 maxlength=4 option=regNum required label="수령자 핸드폰번호" readonlyCheck> -
		<input type=text name=mobileReceiver[] value="<?php echo $TPL_VAR["mobile"][ 2]?>" size=4 maxlength=4 option=regNum required label="수령자 핸드폰번호" readonlyCheck>
		</td>
	</tr>
	<tr>
		<td>남기실 말씀</td>
...
<?php if($TPL_VAR["view_aboutdc"]){?>
	<tr>
		<td>어바웃 쿠폰</td>
		<td><span style="width:145;text-align:right"><?php echo number_format($TPL_VAR["about_coupon"])?></span> 원</td>
	</tr>
<?php }?>
...
<?php if($GLOBALS["sess"]){?>
	<tr>
		<td>회원할인</td>
		<td><span id='memberdc' style="width:145;text-align:right"><?php echo number_format($TPL_VAR["cart"]->dcprice)?></span> 원</td>
	</tr>
	<tr>
		<td>쿠폰 적용</td>
		<td>
		...
		<tr>
			...
			<a href="javascript:popup('../proc/popup_coupon.php',600,700)"><img src="/shop/data/skin/damoashop/img/common/btn_coupon.gif" align=absmiddle hspace=2></a><span id="del_coupon" style="visibility:hidden"><a href='javascript:del_coupon();'><img src="/shop/data/skin/damoashop/img/common/btn_coupon_del.gif" align=absmiddle hspace=2></a></span>
			</td>
		</tr>
		<tr>
	...
	<tr>
		...
		<tr>
			...
			<td style="padding-left:3px">
			<input type=text name=emoney id="emoney" size=12 style="text-align:right" value=0 onblur="chk_emoney(this);" onkeyup="calcu_settle();" onkeydown="if (event.keyCode == 13) {return false;}"  <?php if($GLOBALS["set"]["emoney"]["totallimit"]>$TPL_VAR["cart"]->goodsprice){?>disabled<?php }?>> 원
...
<?php if($GLOBALS["sess"]&&$TPL_VAR["ncash"]["save_mode"]=='choice'){?>
			...
			<input type="radio" name="save_mode" value="ncash" onclick="javascript:check_naverNcashUseAble();"> 네이버 마일리지 <img src="<?php echo $GLOBALS["cfg"]["rootDir"]?>/proc/naver_mileage/images/n_mileage_on.png"/><br/>
<?php }elseif($GLOBALS["sess"]&&$TPL_VAR["ncash"]["save_mode"]=='both'){?>
			...
			<input type="radio" name="save_mode" value="both" onclick="javascript:check_naverNcashUseAble();" checked> 쇼핑몰 적립금 과 네이버 마일리지 <img src="<?php echo $GLOBALS["cfg"]["rootDir"]?>/proc/naver_mileage/images/n_mileage_on.png"/> 동시 적립<br/>
<?php }else{?>
			<input type="radio" name="save_mode" value="unused" onclick="naver_mileage_initialize(this);"> 네이버 마일리지 사용/적립 안함<br/>
			<input type="radio" name="save_mode" value="ncash" onclick="javascript:check_naverNcashUseAble();"> 네이버 마일리지 <img src="<?php echo $GLOBALS["cfg"]["rootDir"]?>/proc/naver_mileage/images/n_mileage_on.png"/><br/>
<?php }?>
			<div id="save_button"><img src="/shop/data/skin/damoashop/img/nmileage/n_mileage_use.gif" onClick="javascript:cash_save_use();" class="mileage_button"> 버튼을 클릭해서 <span id="naver-mileage-accum-rate" style="color: #1ec228; font-weight: bold;"></span> 적립받고 사용하세요 <img class="n_mileage" src="/shop/data/skin/damoashop/img/nmileage/n_mileage_info2.gif" onclick="mileage_info();"></div>
...
<?php }?>
	...
	<tr>
		<td>총 결제금액</td>
		<td><span id=paper_settlement style="width:146px;text-align:right;font:bold 14px tahoma; color:FF6C68;"><?php echo number_format($TPL_VAR["cart"]->totalprice-$TPL_VAR["cart"]->dcprice-$TPL_VAR["cart"]->special_discount_amount)?></span> 원</td>
	</tr>
	</table>
	</td>
</tr>
...
<?php if($TPL_VAR["Payco"]){?>
	<tr>
		<td>PAYCO 결제</td>
		<td class="noline"><input type=radio name=settlekind value="t" onclick="input_escrow(this,'N');check_naverNcashUseAble();"><?php echo $TPL_VAR["Payco"]?></td>
	</tr>
<?php }?>
...
<?php if($GLOBALS["sess"]["level"]< 2||$GLOBALS["sess"]["level"]== 100){?>
...
<?php if($GLOBALS["set"]["use"]["u"]){?>
		<input type=radio name=settlekind value="u" onclick="input_escrow(this,'N')"> CUP (중국카드)
<?php }?>
<?php if($GLOBALS["set"]["use"]["y"]){?>
		<input type=radio name=settlekind value="y" onclick="input_escrow(this,'N')"> 옐로페이
<?php }?>
		</td>
	</tr>
...
<?php if($TPL_VAR["useIpayPg"]===true){?>
	<tr>
		<td>옥션 iPay</td>
		<td class="noline">
			<input type=radio name=settlekind value="i" onclick="input_escrow(this,'Y')"> 아이페이 안전결제
...
<?php if($GLOBALS["egg"]["scope"]=="P"){?>
			<div class=noline>&#149; 전자보증보험 발급여부 : <input type=radio name=eggIssue value="Y" onclick="egg_required()"> 발급 <input type=radio name=eggIssue value="N" onclick="egg_required()" checked="checked"> 미발급</div>
<?php }?>
...
<!-- <input type="image" src="/shop/data/skin/damoashop/img/common/btn_payment.gif"> -->
<a href="javascript:submitForm()"><img src="/shop/data/skin/damoashop/img/common/btn_payment.gif"></a>
<img src="/shop/data/skin/damoashop/img/common/btn_cancel.gif" onclick="history.back()" style="cursor:pointer">
</div>
...
var select_delivery = 1; // 배송지선택 전역변수
<?php if($GLOBALS["sess"]){?>
if(!address) {
	document.getElementById("deli_select3").checked = true;
	select_delivery = 3;
	ctrl_field(3);
}
<?php }?>
...
<?php if($GLOBALS["sess"]){?>
	if(select_delivery == 1 || select_delivery == 2) {
		for (var i=0;i<document.frmOrder.elements.length;i++) {
			chkEl = document.frmOrder.elements[i];
			if (chkEl.getAttribute("readonlyCheck")!=null && !chkEl.value && chkEl.id != 'zonecode' && chkEl.id != 'address_sub') {
...
function getDelivery(){
	...
	gd_ajax({
		...
		type : 'get',
		param : "zipcode="+zipcode+"&deliPoli="+deliPoli+"&coupon="+coupon+"&emoney="+emoney+"&mode="+mode+"&road_address="+road_address+"&address="+address+"&address_sub="+address_sub,
...
function cash_save_use(){
	...
	special_discount_amount = _ID('special_discount_amount') ? uncomma(_ID('special_discount_amount').innerHTML) : 0,
	max_ncash_use = (goodsprice + delivery_price) - exception_price - member_dc - coupon - emoney - special_discount_amount;
...
function deliveryAddCheck(nm, obj) {
	//var _val = document.getElementById(nm).checked;
	var _val = obj.checked;
	if(_val) {
		gd_ajax({
			url : './order_delivery_indb.php',
			type : 'POST',
			param : "&mode=check",
			success : function(rst){
				if(rst) {
					alert("등록가능한 배송지를 모두 등록하였습니다.\n배송지 목록에서 배송지 삭제 후 추가하실 수 있습니다.");
					document.getElementById("delivery_add").checked = false;
					document.getElementById("delivery_default").checked = false;
				}
			}
		});
	}
...
function readonlyEvent() {
	...
	for (var i=0;i<document.frmOrder.elements.length;i++) {
		...
		if (chkEl.getAttribute("readonlyCheck")!=null) {
			...
			chkEl.className = "";
			if (select_delivery == 1 || select_delivery == 2) {
				chkEl.className = org_class+" dark_gray";
				chkEl.readOnly = true;
			} else {
