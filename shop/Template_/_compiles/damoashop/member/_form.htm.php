...
#pwdManual p { background:#F1F1F1 url('/shop/data/skin/damoashop/img/common/blt_tip_gr.gif') no-repeat 10px center;margin:0;padding:12px 10px 12px 50px;color:#373737;font-weight:bold;}
...
<tr>
	<td><div style="height:160px; padding:20px; overflow-y:scroll;"><?php echo $this->define('tpl_include_file_1',"/service/_private2.txt")?> <?php $this->print_("tpl_include_file_1",$TPL_SCP,1);?></div></td>
	<td width="130" style="padding:17px; background:#f1f1f1; border-left:1px solid #ccc;">
		<input type="radio" name="private2" value="y" <?php echo $GLOBALS["checked"]["private2"]["y"]?> style="border:0; background:;" label="개인정보 제3자 제공 관련" msgR="동의 여부를 체크해주세요."> 동의합니다<br />
		<input type="radio" name="private2" value="n" <?php echo $GLOBALS["checked"]["private2"]["n"]?> style="border:0; background:;" label="개인정보 제3자 제공 관련" msgR="동의 여부를 체크해주세요."> 동의하지 않습니다
	</td>
</tr>
</table>
<?php }?>
...
<table width="100%" height="160" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #ccc; margin-bottom:25px;">
<tr>
	<td><div style="height:160px; padding:20px; overflow-y:scroll;"><?php echo $this->define('tpl_include_file_2',"/service/_private3.txt")?> <?php $this->print_("tpl_include_file_2",$TPL_SCP,1);?></div></td>
	<td width="130" style="padding:17px; background:#f1f1f1; border-left:1px solid #ccc;">
		<input type="radio" name="private3" value="y" <?php echo $GLOBALS["checked"]["private3"]["y"]?> style="border:0; background:;" label="개인정보취급 위탁 관련" msgR="동의 여부를 체크해주세요."> 동의합니다<br />
		<input type="radio" name="private3" value="n" <?php echo $GLOBALS["checked"]["private3"]["n"]?> style="border:0; background:;" label="개인정보취급 위탁 관련" msgR="동의 여부를 체크해주세요."> 동의하지 않습니다
	</td>
</tr>
</table>
<?php }?>
...
<tr>
	...
	<td width="130" style="padding:17px; background:#f1f1f1; border-left:1px solid #ccc;">
		<input type="radio" name="consent[<?php echo $TPL_V1["sno"]?>]" value="y" <?php if($TPL_V1["consentyn"]=='y'){?>checked="checked"<?php }?> style="border:0; background:;" label="<?php echo $TPL_V1["title"]?>" msgR="동의 여부를 체크해주세요."> 동의합니다<br />
		<input type="radio" name="consent[<?php echo $TPL_V1["sno"]?>]" value="n" <?php if($TPL_V1["consentyn"]=='n'){?>checked="checked"<?php }?> style="border:0; background:;" label="<?php echo $TPL_V1["title"]?>" msgR="동의 여부를 체크해주세요."> 동의하지 않습니다
	</td>
</tr>
</table>
<?php }}?>
...
<div><img src="/shop/data/skin/damoashop/img/common/join_txt_04.gif" border=0 align=absmiddle><font color=FF6000 >* </font><font class=small><b>필수입력사항</b></font></div>
...
<?php if(!$GLOBALS["sess"]){?>
	...
	<a href="javascript:chkId()"><img src="/shop/data/skin/damoashop/img/common/btn_idcheck.gif" border=0 align=absmiddle></a>
<?php }else{?>
...
<tr>
	...
	<!-- 비밀번호 버튼 -->
	<td class="memberCols2">
		<a href="javascript:void(0);" onclick="openPasswordField()"><img src="/shop/data/skin/damoashop/img/common/btn_change_pwd.gif" align="absmiddle" alt="비밀번호 변경" /></a>
		<a href="javascript:void(0);" onclick="_ID('pwdManual').style.display='block';"><img src="/shop/data/skin/damoashop/img/common/btn_help_pwd.gif" align="absmiddle" alt="비밀번호 도움말" /></a><br />
		<!-- 비밀번호 도움말 -->
		<div id="pwdManual">
			<p>
			...
			<p class="close">
				<a href="javascript:void(0);" onclick="_ID('pwdManual').style.display='none';"><img src="/shop/data/skin/damoashop/img/common/close_x.gif" /></a>
			</p>
		</div>
	</td>
</tr>
...
<?php if(strlen($TPL_VAR["password"])> 0){?>
		<tr>
...
<tr>
	<td class=memberCols1><font color=FF6000>*</font> 비밀번호</td>
	<td class=memberCols2>
	<input type=password name=password required fld_esssential option=regPass label="비밀번호" onfocus="checkPassword(this)" onkeyup="checkPassword(this)" onblur="emptyPwState()" maxlength="16">
...
<tr>
	...
	<td class=memberCols2>
	<input type=password name=password2 required fld_esssential option=regPass label="비밀번호" maxlength="16">
	</td>
</tr>
<?php }?>
<tr><td colspan=2 height=1 bgcolor="#DEDEDE" style="padding:0px;"></td></tr>
...
<tr>
	...
	<a href="javascript:chkNickname()"><img src="/shop/data/skin/damoashop/img/common/btn_nickcheck.gif" border=0 align=absmiddle></a>
	</td>
</tr>
<tr><td colspan=2 height=1 bgcolor="#DEDEDE" style="padding:0px;"></td></tr>
<?php }?>
...
<?php if(!$GLOBALS["sess"]){?><input type=hidden name=chk_email <?php echo $GLOBALS["required"]["email"]?> label="이메일중복체크"><?php }?>
	<a href="javascript:void(0)" onClick="chkEmail()"><img src="/shop/data/skin/damoashop/img/common/btn_mailcheck.gif" border=0 align=absmiddle></a>
<?php if($GLOBALS["checked"]["useField"]["mailling"]){?>
	<span class=noline style="padding-left:10px"><input type=checkbox name=mailling <?php echo $GLOBALS["checked"]["mailling"]?>><span style="font:8pt 돋움;color:#007FC8" >광고성 정보,이벤트메일수신</span></span>
...
<tr>
	...
	<tr>
		<td>
		<input type=text name="zonecode" id="zonecode" size=5 class="line" readonly value="<?php echo $TPL_VAR["zonecode"]?>" label="우편번호">
		...
		<a href="javascript:popup('../proc/popup_address.php',500,432)"><img src="/shop/data/skin/damoashop/img/common/btn_zipcode.gif" border=0 align=absmiddle></a>
		</td>
	</tr>
	<tr>
		<td>
...
<tr>
	...
	<td class=memberCols2>
	<input type=text name=mobile[] id="mobile0" value="<?php echo $TPL_VAR["mobile"][ 0]?>" size=4 maxlength=4 <?php echo $GLOBALS["required"]["mobile"]?> option=regNum label="핸드폰" <?php echo $TPL_VAR["mobileReadonly"]?>> -
	<input type=text name=mobile[] id="mobile1" value="<?php echo $TPL_VAR["mobile"][ 1]?>" size=4 maxlength=4 <?php echo $GLOBALS["required"]["mobile"]?> option=regNum label="핸드폰" <?php echo $TPL_VAR["mobileReadonly"]?>> -
	<input type=text name=mobile[] id="mobile2" value="<?php echo $TPL_VAR["mobile"][ 2]?>" size=4 maxlength=4 <?php echo $GLOBALS["required"]["mobile"]?> option=regNum label="핸드폰" <?php echo $TPL_VAR["mobileReadonly"]?>>
<?php if($TPL_VAR["hpauthDreamyn"]=='y'&&$TPL_VAR["hpauthDreammoduseyn"]=='y'){?><a onclick="gohpauthDream()" style="cursor:pointer;"><img src="/shop/data/skin/damoashop/img/common/btn_change_handphone.gif" border=0 align=absmiddle></a><?php }?>
<?php if($GLOBALS["checked"]["useField"]["sms"]){?>
	<span class=noline style="padding-left:10px"><input type=checkbox name=sms <?php echo $GLOBALS["checked"]["sms"]?>><span style="font:8pt 돋움;color:#007FC8" >광고성 정보,이벤트SMS수신</span></span>
...
<div style="width:100%" class=noline><input type=image src="/shop/data/skin/damoashop/img/common/btn_join.gif">
<img src="/shop/data/skin/damoashop/img/common/btn_back.gif" border=0 onClick="history.back()" style="cursor:pointer;"></div>
</td>
</tr>
</table>
<?php }else{?>
...
<tr>
<td id=avoidDbl align=center height=100>
<div style="width:100%" class=noline><input type=image src="/shop/data/skin/damoashop/img/common/btn_modify_info.gif">
<img src="/shop/data/skin/damoashop/img/common/btn_back.gif" border=0 onClick="history.back()" style="cursor:pointer;"></div>
</td>
</tr>
</table>
...
<?php if($TPL_V1["requiredyn"]=='y'){?>
	if (chkRadioSelect(f, "consent[<?php echo $TPL_V1["sno"]?>]", "y", "[<?php echo $TPL_V1["title"]?>]에 동의를 하셔야 회원정보수정이 가능합니다.") === false) {
		return false;
	}
<?php }?>
<?php }}?>
...
<?php if($TPL_VAR["hpauthDreamyn"]=='y'&&$TPL_VAR["hpauthDreammoduseyn"]=='y'){?>
function gohpauthDream(){ //휴대폰본인인증
	...
	ifrmHpauth.location.href=protocol+"//hpauthdream.godo.co.kr/module/NEW_hpauthDream_Main.php?callType=modifymember&shopUrl="+callbackUrl+"&cpid=<?php echo $TPL_VAR["hpauthDreamcpid"]?>";
	return;
}
<?php }?>
</script>