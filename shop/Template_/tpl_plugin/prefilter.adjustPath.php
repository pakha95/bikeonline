...
// convert to relative path :: for peculiar usage requested by some user.
	...
	if ($type==='relative') {
		...
		$ret = implode('', $split);
		if(preg_match( '/data\/lib/',$ret)) $ret = str_replace('data/lib','lib',$ret);
		return $ret;
	}
...
// convert to absolute path
	...
	for ($i=1,$s=count($split); $i<$s; $i+=2) {
		...
		// 프로토콜(http, https) 이 정의되지 않은 외부 경로(단, 조정후 리소스가 해당 서버에 존재할 경우 제외) 제외
		$_tmp = explode(DIRECTORY_SEPARATOR,$split[$i]);
		if (preg_match('/^.+(\.[a-zA-Z]{2,3})$/', array_shift($_tmp),$matches) && $matches[0] != $_SERVER['SERVER_NAME']) {
			if (!is_file($tpl_path_prefix.$split[$i])) continue;
		}
